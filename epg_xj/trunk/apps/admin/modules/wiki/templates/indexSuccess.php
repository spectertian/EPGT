...
function hrefto(obj){
	if(obj.value){
		if(obj.value.match('^.*delete.*$')){
			if(window.confirm("确定删除吗?")){window.location.href=obj.value;}
		}else{
...
<!--          <form action="" method="get">-->
            ...
            <div class="table_nav">
                <?php include_partial('search', array('c' => $c,'q' => $q,'m' => $m,'tag' => $tag,'users'=>$users))?>
            <div class="paginator">
              <span class="first-page">
                  <a href="<?php echo url_for("wiki/index?c=$c&q=$q&m=$m&tag=$tag&page=".$wiki->getFirstPage());?>">
                  最前页
                  </a>
              </span>
              <span class="prev-page">
                 <a href="<?php echo url_for("wiki/index?c=$c&q=$q&m=$m&tag=$tag&page=".$wiki->getPreviousPage());?>">上一页</a>
              </span>
              <span class="pages">
                ...
                <?php foreach ($links as $key => $value):?>
                    ...
                    <?php else:?>
                        <a href="<?php echo url_for("wiki/index?c=$c&q=$q&m=$m&tag=$tag&page=".$value );?>"><?php echo $value;?></a>
                    <?php endif;?>
                <?php endforeach;?>
              </span>
              <span class="next-page"><a href="<?php echo url_for("wiki/index?c=$c&q=$q&m=$m&tag=$tag&page=".$wiki->getNextPage());?>">下一页</a></span>
              <span class="last-page"><a href="<?php echo url_for("wiki/index?c=$c&q=$q&m=$m&tag=$tag&page=".$wiki->getLastPage());?>">最末页</a></span>
            ...
            <table cellspacing="0">
              ...
              <?php if(isset ($wiki)):?>
                ...
                <tr>
                  ...
                  <td rel="<?php echo $rs->getId();?>" style="text-align: center;">
			          <select name="opt" id="opt['<?php echo $rs->getId()?>']" onchange="hrefto(this)">
			          <option value="">请选择</option>
		              <option value="<?php echo url_for("wiki/recommend?id=".$rs->getId())?>">点播维基推荐</option>
		              <option value="<?php echo url_for("wiki_liverecommend/liverecommend?id=".$rs->getId())?>">直播维基推荐</option>
	                  <option value="<?php echo url_for("wiki/delete?id=".$rs->getId());?>">删除</option>
			          <option value="<?php echo url_for("wiki/resetXunSearch?id=".$rs->getId())?>">重建索引</option>
			          </select>
			      </td>
