...
$(document).ready(function(){
	...
	del.live('click', function() {
			if(confirm('确定删除吗?'))
			{
				...
				$.ajax({
			        url:            "/program_sport/ajax_del?id=" + id ,
			        dataType:       "json",
...
function ajax_update(id ,key)
{
    ...
    $.ajax({
        url:            "/program_sport/ajax_update?id=" + id +"&key="+key+"&value="+value,
        dataType:       "json",
        success:function(data)
        {
            if(data.code == 1)
      ...
      <div id="content">
        <div class="content_inner">
            ...
            <table cellspacing="0" class="adminlist" id="admin_list">
              ...
              <?php foreach($programs as $program):?>
                <tr>
                  ...
                  <td style="cursor:pointer;" class="sf_admin_list_td_name" scope="col"><?php echo $program->getName() ?></th>
            ...
            <h3>tvsou:(<?php echo date('Y-m-d')?>)</h3>
            <table cellspacing="0" class="adminlist" id="admin_list">
              <thead>
                <tr>
                  <th scope="col"  name="name" style="width:50%">名称</th>
                  <th scope="col"  name="channel_id" style="width:30%">频道</th>
                  <th scope="col"  name="time"  style="width:20%">播放时间</th>
                </tr>
              </thead>
