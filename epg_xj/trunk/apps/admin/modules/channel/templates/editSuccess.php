        ...
        <div class="content_inner">
          ...
          <?php include_partial('global/flashes') ?>
           ...
           <?php foreach ($fields as $name => $field): ?>
            <?php //if($name == 'code'&&!$form->isNew()) { continue;} ?>
            ...
            <?php endforeach;?>
                <li>
                    ...
                    <input type="text" name="channel[config]" id="config" value="<?php echo $channel->getConfig(); ?>">
                </li>
                <li>
                    <label for="autosyn">自动同步</label>
                    <select name="channel[autosyn]" id="autosyn">
                        <option value='1' <?php if($channel->getAutosyn()==1) echo 'selected'; ?>>是</option>
                        <option value='0' <?php if($channel->getAutosyn()==0) echo 'selected'; ?>>否</option>
                    </select>
                </li>
                <li>
                    <label for="type">类型</label>
                    <select name="channel[type]" id="type">
                        <option value='0'>请选择</option>
                        <option value='cctv' <?php if($channel->getType()=='cctv') echo 'selected'; ?>>cctv</option>
                        <option value='tv' <?php if($channel->getType()=='tv') echo 'selected'; ?>>tv</option>
                        <option value='local' <?php if($channel->getType()=='local') echo 'selected'; ?>>local</option>
                        <option value='edu' <?php if($channel->getType()=='edu') echo 'selected'; ?>>edu</option>
                    </select>
                </li>
                <li>
                    <label for="province">省</label>
                    <input type='text' name='channel[province]' id='province' value='<?php echo $channel->getProvince(); ?>'>
                </li>
                <li>
                    <label for="city">市</label>
                    <input type='text' name='channel[city]' id='city' value='<?php echo $channel->getCity(); ?>'>
                </li>
            </ul>
                </div>
              </div>
