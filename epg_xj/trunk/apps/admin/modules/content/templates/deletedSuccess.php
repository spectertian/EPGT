...
function textToActions(action,is_new){
	...
	if(action == 'edit'){
		...
		second_A.text(" 还原 ").attr({'href':'#'}).unbind('click').bind('click',function(){
...
function program_deleteajax(ids){
    $.ajax({
        ...
        type: 'post',
        dataType: 'text',
        data: {'ids': ids,'type':'reduction'},
        success: function(data){
            //alert(data);
            location.href='<?php echo url_for('content/import?deleted=1')?>'; 
        }
    });
    ...
    admin_form.submit();
      ...
      <div id="content">
        <div class="content_inner">            
            <?php include_partial('toolbarList',array('deleted'=>$deleted,'pageTitle'=>$pageTitle))?>
            ...
            <form action='/content/import' method='post'>
              ...
              <input type='submit' value='查询'><a href="/content/import?deleted=1">重置返回列表</a>
            <a href='/content/import' style='margin-left:20px'><b  style='color:red'>查看未删除</b></a>
            </form>
                <div class="paginator">
                  <span class="first-page">
                      <a href="<?php echo url_for("content/import?deleted=1&page=".$imports->getFirstPage());?>">
                      最前页
                      </a>
                  </span>
                  <span class="prev-page">
                     <a href="<?php echo url_for("content/import?deleted=1&page=".$imports->getPreviousPage());?>">上一页</a>
                  </span>
                  <span class="pages">
                    ...
                    <?php foreach ($links as $key => $value):?>
                        ...
                        <?php else:?>
                            <a href="<?php echo url_for("content/import?deleted=1&page=".$value );?>"><?php echo $value;?></a>
                        <?php endif;?>
                    <?php endforeach;?>
                  </span>
                  <span class="next-page"><a href="<?php echo url_for("content/import?deleted=1&page=".$imports->getNextPage());?>">下一页</a></span>
                  <span class="last-page"><a href="<?php echo url_for("content/import?deleted=1&page=".$imports->getLastPage());?>">最末页</a></span>
            ...
            <table cellspacing="0" id="admin_list" class="adminlist">
              ...
              <?php if(isset ($imports)):?>
                ...
                <tr edit="0">
                  ...
                  <td class="noEdit actions edit" rel="<?php echo $import->getId();?>" backvalue="保存|取消"><a href="#" onclick="changeTR($(this),'newProgram',false);return false;" >编辑</a> | <a onClick="if(confirm('确定还原吗？')){program_deleteone($(this),1);return false;}">还原</a></td>
                </tr>
                <?php endforeach;?>
              <?php endif;?>
              </tbody>
