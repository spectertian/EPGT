...
function delRecommendRsId(rs_id) {
    if(rs_id.length==0){
        ...
        $.ajax({
            type: "POST",
            data: {'id' : rs_id},
            url: '<?php echo url_for('wiki_package/delete')?>',
...
<!--          <form action="" method="get">-->
            <?php include_partial('toolbarList',array('pageTitle'=>'维基包列表'))?>
            <?php include_partial('global/flashes') ?>
            <div class="table_nav">
            <?php include_partial('select', array('m' => $m,'show'=>$show))?>
            
<form method="post" name="adminForm" action="<?php echo url_for('wiki_package/index');?>/batch">
            <div class="paginator">
              <span class="first-page">
                  <a href="<?php echo url_for("wiki_package/index?m=$m&s=$show&page=".$pager->getFirstPage());?>">
                  最前页
                  </a>
              </span>
              <span class="prev-page">
                 <a href="<?php echo url_for("wiki_package/index?m=$m&s=$show&page=".$pager->getPreviousPage());?>">上一页</a>
              </span>
              <span class="pages">
                  <?php foreach ($pager->getLinks(5) as $page ):?>
                        ...
                        <?php else:?>
                            <a href="<?php echo url_for("wiki_package/index?m=$m&s=$show&page=".$page);?>"><?php echo $page;?></a>
                        <?php endif;?>
                  <?php endforeach;?>
              </span>
              <span class="next-page"><a href="<?php echo url_for("wiki_package/index?m=$m&s=$show&page=".$pager->getNextPage());?>">下一页</a></span>
              <span class="last-page"><a href="<?php echo url_for("wiki_package/index?m=$m&s=$show&page=".$pager->getLastPage());?>">最末页</a></span>
                ...
                <?php if(isset ($pager)):?>
                  <?php foreach ($pager->getResults() as $i => $rs): ?>
                            <tr>
                              ...
                              <td><a href="<?php echo url_for('wiki_package/edit?id='.$rs->getId());?>"><?php echo $rs->getName();?></a></td>
                              <td><?php 
                              $quyu=array('tcl_index_update'=>'tcl首页今日更新');
                              ...
                              <td>
                                  ...
                                  <a href="<?php echo url_for("wiki_package/edit?id=".$rs->getId())?>" class="edit">编辑</a> | 
