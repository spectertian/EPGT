...
class Common
{
    ...
    static function get_filename_rand($file_name) {
        $file_ext = Common::get_file_ext($file_name);
        return time().rand(100, 999).".".$file_ext;
    }
	...
	static function post_json($url,$post_data,$timeout = 20) 
	{
		if(is_array($post_data)){
			$post_data = json_encode($post_data);
		}
...
//         }
    ...
    static function getBaiduVedioList($id,$site)
    {
        $id = intval($id);
        $sites = array("iqiyi.com","pptv.com","pps.tv","letv.com","qq.com","sohu.com","tudou.com","wasu.cn","cntv.cn");
        if(isset($sites[$site])) {
	...
	{
        $strs = array("A","B","C","D","E","F","G","H","J","K","L","M","N","P","Q","R","S","T","U","V","W","X","Y","Z","2","3","4","5","6","7","8","9");
		$hash = '';
        $strs_length = count($strs)-1;
		for($i = 0; $i < $length; $i++) 
		{
            $rand = mt_rand(0, $strs_length);
			$hash .= $strs[$rand];
		}
		return $hash;;
	}
    /**
     * 
     * @deprecated 根据tvsou_id 获取tvsou的title
     * @param string $id
     * @return string $title
     * @author tianzhongsheng-ex 2013-08-19 17:10:00
     */
    static function getTvsouTitleByID($id)
    {
    	$fistUrl = substr($id,0,-2);
    	$url = "http://jq.tvsou.com/introhtml/$fistUrl/index_$id.htm";
