...
class ThemeForm extends BaseThemeForm
{
  public function configure()
  {
        ...
        $this->setWidget("scene", new sfWidgetFormSelect(array('choices' => array('tcl'=>'TCL', 'changhong'=>'长虹'))));
        $this->widgetSchema->setLabel('publish', '状态：');
         $this->setWidget("model", new sfWidgetFormSelect(array('choices' => array('default'=>'默认', 'tcl2.0'=>'长虹2.0'))));
        $this->widgetSchema->setLabel('model', '模型选择：');
        $this->widgetSchema->setLabel('scene', '推荐平台');
        $this->setValidator("img", new sfValidatorString(array('required'=>false)));  //img不必须验证
        $this->setValidator("remark", new sfValidatorString(array('required'=>false)));  //img不必须验证
  }
}