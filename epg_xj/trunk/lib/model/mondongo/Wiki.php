...
class Wiki extends \BaseWiki
{
	...
	{        
        ...
        $programRepos = $mongo->getRepository('program');
        $programs = $programRepos->find(array(
        			'query'=>array(
        ...
        //同步推荐维基
        $wrRepos = $mongo->getRepository('WikiRecommend');
        $wrs = $wrRepos->find(array(
        			'query'=>array(
        				'wiki_id'=>(string)$this->getId()
        		)
        ));
    ...
    {   
        ...
        $wikiPackageRepos = $mongo->getRepository('wikiPackage');
        //推荐删除
        ...
        //删除wikipackage
        $wikiPackages = $wikiPackageRepos->find(array('query' => array('wiki_id' => (string)$this->getId())));
        if($wikiPackages) {
            foreach ($wikiPackages as $wikiPackage) {
                $wikiPackage->delete();
            }
        }
    }
            ...
            case 'film':
                ...
                if($videos = $this->getVideos()){
                    foreach($videos as $video) {
                   		...
                   		if($refer == 'qiyi')$data['hasvideo'] = true; //标示是否有视频源
						$sources[] = $refer;
