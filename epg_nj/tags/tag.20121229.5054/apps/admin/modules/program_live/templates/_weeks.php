...
$(document).ready(function(){
    ...
    $("#goToFilters").click(function(){
        ...
        var url = '<?php echo url_for('program_live/index') ?>' + '?date=' + date;
          ...
          <?php
            ...
            for ($i = 0; $i <= 8; ++$i) {
                ...
                echo sprintf('<li><a href="%s" class="%s" link=\'{"channel_id":%s,"date":"%s"}\'>%s</a></li>',
                        url_for('program_live/index').'?date=' . $tmps[$i],  $current_date == $tmps[$i] ? 'active' : '', $channel_id, $tmps[$i], (($i != 0 && $i != 8 ) ? $tmps[$i] : '') . $weeks[$i]);
            }
      ?>
