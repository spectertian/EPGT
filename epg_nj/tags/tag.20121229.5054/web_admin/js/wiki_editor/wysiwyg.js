...
TracWysiwyg.prototype.createAnchor = function(link, label, attrs) {
    ...
    {
        label = label.split(" ");
        //label = (label[1].length > 0 ) ? label[1] : null;
        label = label[1];
    }
...
(function() {
    //var _linkScheme = "[\\w.+-]+[^:a-zA-Z]+";  (?:[\\w.+-/?!#@=:%&]+|[^\\w]+)?
    var _linkScheme = "(?:[\\w.+-]+|[\u4E00-\u9FA5]+)";
    ...
    var _tracLink =  _linkScheme + "[:|\\s](?:" + _quotedString     
...
//    wikiInlineRules.push("!?^\\/\\/");           // 3. italic
    ...
    wikiInlineRules.push("!?\\[\\[(?:"         // 15. [wiki:TracLinks label] or [/relative label] //TEXTAREA TO WYSIWYG 链接主要识别
        + "[/.#][^ \\t\\r\\f\\v[\\]]*|"
        + _linkScheme + "[:\\s](?:" + _quotedString + "|[^\\] \\t\\r\\f\\v]*)|"
        ...
        for ( ; ; ) {
            ...
            if (!/^!/.test(matchText)) {    // start '!'
                ...
                case 18:    // ["internal free link"]
                    ...
                    continue;
                case 19:
                    tmp = contentDocument.createElement("br");
            ...
            if (matchText) {
                ...
                var tmp;
                if (matchNumber == 16 ) {  //BR
                    //tmp = /^!?\[\[br\]\]$/i.test(matchText)
                    tmp = /^!?\\\\$/i.test(matchText)
            ...
            if (match) {
                if (label == match[2]) {
                   // if (match[1] == "wiki" && wikiPageNamePattern.test(match[2])) {
                    if (match[1] == label && wikiPageNamePattern.test(match[2])) {
                        text = match[2];
//                        traclink = "[wiki:" + text + "]";
                        traclink = "[[" + text + " " + label +"]]";
                    }
