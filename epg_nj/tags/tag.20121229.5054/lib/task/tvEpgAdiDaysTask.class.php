...
EOF;
    //symfony tv:EpgAdiDays --dates=7 --channel=GuiZhouTV --nosend=true  //nosend=true时不发送
  }
        ...
        foreach($channels as $channel){
            ...
            for($days = 0; $days < $dates ; $days ++) {
                ...
                if(!isset($options['nosend'])){
                    ...
                    $bkjson = @file_get_contents("c?action=adi11&systemId=epg", false, $context);
                    if(!$bkjson)
                        echo $channel->getName()." 导入失败";
                }else{
                    echo "no send\n";
                }
                sleep(20);  //停3秒，接着发下一天的
            }
            //#########################结束循环
            //break;  
            $channel_num++;  
