        ...
        <div class="content_inner">
            ...
            <div class="table_nav">
             <?php include_partial('search', array('form' => $filters, 'configuration' => $configuration)) ?>
              <div class="paginator">
                <span class="first-page"> <a href="<?php echo url_for('@channel') ?>?page=1&type=<?php echo $type?>">最前页</a></span>
                <span class="prev-page"><a href="<?php echo url_for('@channel') ?>?page=<?php echo $pager->getPreviousPage() ?>&type=<?php echo $type?>">上一页</a></span>
                <span class="pages">
                    <?php foreach ($pager->getLinks() as $page): ?>
                        <?php if ($page == $pager->getPage()): ?>
                         <span class="present"><?php echo $page ?></span>
                        <?php else: ?>
                          <a href="<?php echo url_for('@channel') ?>?page=<?php echo $page ?>&type=<?php echo $type?>"><?php echo $page ?></a>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </span>
                <span class="next-page"><a href="<?php echo url_for('@channel') ?>?page=<?php echo $pager->getNextPage() ?>&type=<?php echo $type?>">下一页</a></span>
                <span class="last-page"><a href="<?php echo url_for('@channel') ?>?page=<?php echo $pager->getLastPage() ?>&type=<?php echo $type?>">最末页</a></span>
                ...
                <tr>
                  ...
                  <th scope="col" class="list_modified_by">
                      <?php if ('sort_id' == $sort[0]): ?>
                        <?php echo link_to(__('排序(小靠前)', array(), 'messages'), '@channel', array('query_string' => 'sort=sort_id&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc'))) ?>
                        <?php echo image_tag('icon/sort_'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'sf_admin'), 'title' => __($sort[1], array(), 'sf_admin'))) ?>
                      <?php else: ?>
                        <?php echo link_to(__('排序(小靠前)', array(), 'messages'), '@channel', array('query_string' => 'sort=sort_id&sort_type=asc')) ?>
                      <?php endif; ?>
                  </th>
                  <th scope="col" class="list_updated_at">电视节目</th>
                </tr>
              </thead>
              <tfoot>
                <tr>
