...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        //$conn = @ftp_connect("172.20.224.163") or die("FTP服务器连接失败"); 
        //@ftp_login($conn,"cep","cep") or die("FTP服务器登陆失败");   
        //$conna = @ftp_connect("10.20.20.132") or die("FTP服务器连接失败"); 
        //@ftp_login($conna,"wangyong","wangyong") or die("FTP服务器登陆失败"); 
        
        $date = date("Y-m-d");
        $file='log/tv_'.date("YmdHis").'.csv';
        $target_file='tv/tv_'.date("YmdHis").'.csv';
        @unlink($file);   
