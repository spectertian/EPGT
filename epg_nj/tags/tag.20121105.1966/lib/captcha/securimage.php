...
//		$this->ttf_file      = './AHGBold.ttf';
        $this->ttf_file      = dirname(__FILE__).'/ttf/arialbd.ttf';
	...
	function saveData()
	{
            $this->user->setAttribute('securimage_code_value', strtolower($this->code)) ;
            $this->user->setAttribute('securimage_code_ctime', time());
...
//		$_SESSION['securimage_code_ctime'] = time();
	...
	function validate()
	{
		// retrieve code from session, if no code exists check sqlite database if supported.
                $securimage_code_value = $this->user->getAttribute('securimage_code_value');
                $securimage_code_ctime = $this->user->getAttribute('securimage_code_ctime');
		if (isset($securimage_code_value) && trim($securimage_code_value) != '') {
			if ($this->isCodeExpired($securimage_code_ctime) == false) {
			  $code = $securimage_code_value;
			}
		...
		if ($code != '') {
			if ($code == $code_entered) {
			  ...
			  $this->user->setAttribute('securimage_code_value','');
			  $this->user->setAttribute('securimage_code_ctime','');
	...
	function saveCodeToDatabase()
	{
		...
		if ($this->use_sqlite_db && $this->sqlite_handle !== false) {
			$ip = $this->getIPHash();
			$time = time();
			$code = $this->user->getAttribute('securimage_code_value'); // hash code for security - if cookies are disabled the session still exists at this point
