...
function showTip(_tip) {
    ...
    if(arguments[2]=='big'){  //为了wiki详情的更多内容设置
        $("#tipInfoSpan").html(_tip);
        $(".tipc").addClass('tipbig');
        $(".tipc h2 span").html('详细信息');
        $(".tipc").find("a")[0].focus();
        $("#tipInfoSpan").css("top",0);
        textnum=_tip.length;  //字数，根据其控制翻页
        pagenum=parseInt(textnum/280);  //页数
        //按任意键隐藏
        $(".tipc").find("a").eq(0)[0].onkeydown=function(evt){
            var evtcode = evt.which ? evt.which : evt.code;
            switch (evtcode) {
				case 0x78:   //上页
				    if(parseInt($("#tipInfoSpan").css("top"))!=0){
						$("#tipInfoSpan").css("top",parseInt($("#tipInfoSpan").css("top"))+300);
					}
					evt.preventDefault();
					break;
				case 0x79:  //下页
				    if(parseInt($("#tipInfoSpan").css("top"))>-300*pagenum){
						$("#tipInfoSpan").css("top",parseInt($("#tipInfoSpan").css("top"))-300);
					}
					evt.preventDefault();
					break;
