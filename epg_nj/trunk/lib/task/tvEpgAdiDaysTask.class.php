 ...
 */
class tvEpgAdiDaysTask extends sfMondongoTask
{
...
The [tv:EpgAdiDays|INFO] task does things.
Call it with:

  [php symfony tv:EpgAdiDays|INFO]
EOF;
    //symfony tv:EpgAdiDays --dates=7 --channel=GuiZhouTV --channelCode=2aeb585ccaca9fa893b0bdfdbc098c7f --nosend=true --update=0  //nosend=true时不发送,update=0发送全部频道
  }
    ...
    { 
        ...
        $crontabLog->setTitle('EpgAdiDays');
            ...
            for($days = 0; $days < $dates ; $days ++) {        
                ...
                if(!isset($options['nosend'])){
                    try{
                        ...
                        //$bkjson = Common::post_url_content("$bkurl?action=adi11&systemId=epg",$channelXML);
                        if(!$bkjson){
                    ...
                    sleep($sleepTime);  //周六停60s,其他时间停10s
                }else{
                    echo "no send\n";
                }
            }
