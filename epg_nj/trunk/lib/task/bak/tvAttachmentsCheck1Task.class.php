 ...
 *  @author: superwen
 */
class attachmentsCheck1Task extends sfBaseTask
{
...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        if (isset($options['file_key'])){
            ...
            $counts = Doctrine::getTable('Attachments')->createQuery()->where('created_at >= ?','2012-11-27 19:18:00')->andWhere('created_at <= ?','2012-12-12 10:18:00')->count();
            $storage = StorageService::get('photo1');
            $i = 0;
            $limit = 100;
            while ($i < $counts) {
                echo $i." ===\n";
                $attmts = Doctrine::getTable('Attachments')->createQuery()->where('created_at >= ?','2012-12-10 19:18:00')->andWhere('created_at <= ?','2012-12-12 10:18:00')->offset($i)->limit($limit)->execute();
