...
//        $wikis = $this->find(array("query" => array("_id" => array('$in' => $wiki_ids))));
    ...
    /**
     * 根据相关标签获取相关的维基 应用 mongo $all 查询，广电专用
     * @param <array> $tags
     * @param <integer> $limit
     * @return <object>
     * @author lifucang 2012-11-19
     */
    public function getWikiByTagsGd(Wiki $wiki, $tags, $limit){
        $wikis=$this->find(array(
                            'query' => array(
                                    '_id' => array('$ne' => $wiki->getId()), //跳过当前这条维基
                                    'model' => $wiki->getModel(),
                                    'tags' => array('$all' => $tags)
    ...
    /**
     * 根据相关标签获取相关的维基 应用 mongo $in 查询
     * @param <array> $tags
     * @param <integer> $limit
     * @return <object>
     * @author lifucang 2013-04-24
     */
    public function getWikiByTag($tags, $limit){
        $wikis=$this->find(array(
                            'query' => array(
                                    'tags' => array('$in' => $tags),
     ...
     */ 
    public function getWikiByTitle($wikiSlug,$model=null){
        //$memcache = tvCache::getInstance();
        //$memcache_key = md5('getWikiByTitle'.$wikiSlug);
        //$wiki = $memcache->get($memcache_key);
        
        //if(!$wiki){
            if($model){
                $query=array("title"=>$wikiSlug,'model'=>$model);
            }else{
                $query=array("title"=>$wikiSlug);
            }
            $wiki =  $this->findOne(array("query"=>$query));
            /*
            if(!$wiki){
                if($model){
                    $query=array("alias"=>$wikiSlug,'model'=>$model);
                }else{
                    $query=array("alias"=>$wikiSlug);
                }
                $wiki =  $this->findOne(array("query"=>$query));
            } 
            */          
     ...
     */ 
    public function getWikiIdByTitle($wikiSlug,$model=null){
        if($model){
            $query=array("title"=>$wikiSlug,'model'=>$model);
        }else{
            $query=array("title"=>$wikiSlug);
     ...
     * @param <type> $wikiSlug
     * @return <type> object
     * @author lifucang (2013-04-02)
     */ 
    public function getOneWikiByTitle($wikiSlug){
        $wikiNum =  $this->count(array("title"=>$wikiSlug));
        if($wikiNum==1){
    ...
    public function area_replace($text)
    {
    	...
    	{
    		$text = preg_replace('/area:华语/','(area:中国大陆 OR area:大陆 OR area:中国 OR area:澳门  OR area:香港)',$text);
    		return $text;
    	}
    	if(preg_match('/area:美国/',$text))
    	{
    		$text = preg_replace('/area:美国/','(area:美国 OR area:欧美)',$text);
    		return $text;
    	}
    	if(preg_match('/area:欧洲/',$text))
    	{
    		$text = preg_replace('/area:欧洲/','(area:欧洲 OR area:欧美)',$text);
    		return $text;
    	}
    	if(preg_match('/area:其它/',$text))
    	{
    		$text = preg_replace('/area:其它/',' -area:中国大陆 -area:大陆 -area:美国 -area:日本 -area:欧洲 -area:韩国 -area:香港 -area:澳门 -area:欧美',$text);
    		return $text;
    	}
    	return $text;
     ...
     * @author wangnan
     */
    public function rebuildXunSearchDocument($query=array()) 
    {        
        require_once '/usr/local/xunsearch/sdk/php/lib/XS.php';
        ...
        $add_num=0;
	    while ($i < $wiki_count) {
	        $wikis = $this->find(array("query"=>$query,"sort" => array("_id" => 1), "skip" => $i, "limit" => 50));
            $index->openBuffer();
	        foreach ($wikis as $wiki) 
	        {
	        	//printf("%s\n", iconv('utf-8','gb2312',$wiki->getTitle()));
				...
				$data = array(
                    ...
                    'updated_at'=>$wiki->getUpdatedAt()?$wiki->getUpdatedAt()->getTimestamp():0,
                    'hasvideo' => $wiki->getHasVideo()>0?true:false,
                    'video_update'=>$wiki->getVideoUpdate()?$wiki->getVideoUpdate()->getTimestamp():0,
		        );
		        unset($xun_tag);
