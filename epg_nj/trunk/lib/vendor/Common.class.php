...
class Common
{
    static function get_url_content($url, $timeout = 5) {   
    ...
    static function post_url_content($url, $content,$timeout = 5) {   
	...
	static function post_json($url,$post_data,$timeout = 5) 
	{
		if(is_array($post_data)){
			$post_data = json_encode($post_data);
		}
        ...
        /*
        $header = "Content-type: text/plain"; //定义content-type为xml
        $ch = curl_init();                  //初始化curl
        curl_setopt($ch, CURLOPT_URL, $url);//设置链接
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);//设置是否返回信息
        curl_setopt($ch, CURLOPT_HTTPHEADER, $header);//设置HTTP头
...
//         }
    ...
    static function getCpgURL($contented, $playtype = 0, $backurl = '')
    {
        $submit_url = sfConfig::get("app_cpg_url")."?clientid=01006608470056014&playtype=".$playtype."&startpos=0&devicetype=6&rate=0&hasqueryfee=n&contented=".$contented; 
        $curl = curl_init();          
        curl_setopt($curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC ) ; 
        curl_setopt ($curl, CURLOPT_RETURNTRANSFER, 1); 
        curl_setopt($curl, CURLOPT_USERPWD, sfConfig::get("app_cpg_username").":".sfConfig::get("app_cpg_password")); 
    ...
    static function getVodURL($contented, $playtype = 0, $backurl = '')
    {
        $submit_url = sfConfig::get("app_cpg_url")."?clientid=01006608470056014&playtype=".$playtype."&startpos=0&devicetype=6&rate=0&hasqueryfee=n&contented=".$contented; 
        $curl = curl_init();          
        curl_setopt($curl, CURLOPT_HTTPAUTH, CURLAUTH_BASIC ) ; 
    ...
    static function englishGenres($type=null){
        $englishGenres = array(
            "爱情" => "Romance",
            "古装" => "Ancient costume",
            "家庭" => "Family",
         ...
         if($type){
            return $englishGenres[$type];
         }else{
            return $englishGenres;
         }
    }    
    static function englishGenres_bak($type=null){
        $englishGenres = array(
            "电视剧" => "Series",
        	"电影" => "Movie",
        	"体育" => "Sports",
    ...
    /*
     * 根据字符串获取敏感词数组
     * param $words:用,分隔开的字符串
     */
    static function getSensitiveWords($words){
