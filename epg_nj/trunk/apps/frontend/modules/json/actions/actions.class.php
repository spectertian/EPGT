...
//                break; 
	...
	/**
     * 广电接口
     * 获取当天更新的wiki的Id（GetWikisDayGd的精简版）
     * @author lifucang 
     */
	public function GetWikiIdDayGd($args)
	{
        ...
        $endtime=new MongoDate(mktime(23, 59, 59, date('m'), date('d'), date('Y')));
    	$mongo = sfContext::getInstance()->getMondongo();
		$WikiRepository = $mongo->getRepository('wiki');
	...
	public function GetWikisDayGd($args)
	{
        ...
        foreach($wikis as $wiki) 
		{
			...
			$nodeArray['wiki'][$i] = array(
                            ...
                            'slug'  => $wiki->getSlug(),
                            'model'   => $model,
                            'description' => $wiki->getContent(),
                            'cover' => $wiki->getCover(),
                            'screens' => $wiki->getScreens(),
            );
            if ($model == 'actor') {
                ...
                $nodeArray['wiki'][$i]['info'] = array(
                    "director" => $wiki->getDirector()?$wiki->getDirector():'',
                    "starring" => $wiki->getStarring()?$wiki->getStarring():'',
                    "tags" => $wiki->getTags()?$wiki->getTags():'',
                    "country" => $wiki->getCountry()?$wiki->getCountry():'',
                    "language" => $wiki->getLanguage()?$wiki->getLanguage():'',
                    "rating" => $wiki->getRating()?$wiki->getRating():'',
                    "released" => $wiki->getReleased()?$wiki->getReleased():'',
                    "like_num" => $wiki->getLikeNum()?$wiki->getLikeNum():'',
                    "dislike_num" => $wiki->getDislikeNum()?$wiki->getDislikeNum():'',
                    "source" => $wiki->getSource()?$wiki->getSource():'',
                );
            }
            $i++;
