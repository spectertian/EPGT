...
<div class="wapper">
	<div class="bg">
		...
		<h1 class="titbg"><?php echo $wiki->getTitle()?><?php if($videoCount>6):?>(<?php echo $videoCount?>期)<?php endif;?></h1>
			...
			<div class="tvl">
				<div class="tvinfo">
					<ul>
						...
						<li class="hides">国家：<?php echo $wiki->getCountry()?></li>
						<li class="hides">类型：
				...
				<div class="playlibrary">
					...
					<div class="itemnum" id="myvideo">
						<div class="hidenum">
                           ...
                           ?>
									<li class="videohs <?php echo ceil($j/2); ?>" id="<?php echo $j; ?>" value="<?php echo ceil($j/6); ?>"><a href="javascript:void(0);"  onclick="playWikiVideo('<?php echo $asset_id;?>','<?php echo $video->getReferer()?>');return false;"><span><big><?php echo $title;?></big></span></a></li>
                            <?php
                                	  $j++;
                                	  endforeach;
                            ?>
...
function showVideos(num,wiki_id,refer,hd){
    ...
    $.ajax({
        ...
        type: 'post',
        data: {'num': num,'wiki_id':wiki_id,'model':'television','refer':refer,'hd':hd},
        success: function(data){
		...
		this.onmouseover=function(){
			onkeydown=function(event){
				...
				if((event.keyCode == 40||event.keyCode == 29) && ischange){
					...
					if(page >1){
						page = page-1;
						focusid = pagesize*page-(pagesize-1);
					}
                    //showTip(focusid);
					$('#'+focusid).find('a')[0].focus();
				}
