...
class jsonActions extends sfActions
{
	...
	public function __construct($context=null, $moduleName=null, $actionName=null)
    {
		...
		$this -> action_array = array(      
                            ...
                            'GetAttachments',
							'GetWikiUrlByPackageId',
                            'GetContentInject',             //获取ADI数据
							'GetContentImport',             //获取ADI处理数据
                            );
    }
...
//                break; 
    ...
    /**
     * 广电接口
     * 根据package_id sp_id 获取 wiki_id url
     * @editor majun 2013_03_18
     */
    public function GetWikiUrlByPackageId($params)
    {
    	$mongo  = sfContext::getInstance()->getMondongo()->getRepository('ContentImport');
    	$content = $mongo->findOne(array('query' => array('from_id' => strval($params['package_id']),'provider_id'=> $params['sp_id'],'wiki_id'=>array('$exists'=>true))));
    	if($content){
    		$nodeArray = $this->getErrArray(0,'');
    		$nodeArray['url'] = 'http://'.$_SERVER["HTTP_HOST"].'/wiki/show/id/'.$content->getWikiId();
    	}else {
    		$nodeArray = $this->getErrArray(-1,'没有查询到相关wiki');
    	}
        return $nodeArray;
    }
	...
	/**
     * 广电接口
     * 获取指定时间创建的inject数据--默认昨天
     * @author lifucang 2013-05-27
     */
	public function GetContentInject($args)
	{
		$nodeArray = array();
        $starttime = $args['start_time']?new MongoDate(strtotime($args['start_time'])):new MongoDate(mktime(0, 0, 0, date('m'), date('d')-1, date('Y')));
        $endtime=$args['end_time']?new MongoDate(strtotime($args['end_time'])):new MongoDate(mktime(23, 59, 59, date('m'), date('d')-1, date('Y')));
        $mongo = sfContext::getInstance()->getMondongo();
		$Repository = $mongo->getRepository('ContentInject');
        $injects = $Repository->find(array('query'=>array('created_at'=>array('$gte' => $starttime,'$lte' => $endtime))));
		$nodeArray = $this->getErrArray(0,'',count($injects));
        foreach($injects as $inject) 
		{
			$nodeArray['injects'][] =  array(
	...
	public function GetContentImport($args)
	{
        ...
        $imports = $Repository->find(array('query'=>array('created_at'=>array('$gte' => $starttime,'$lte' => $endtime))));
