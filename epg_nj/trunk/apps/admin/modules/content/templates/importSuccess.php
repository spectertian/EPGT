...
function textToActions(action,is_new){
    ...
    div.append(first_A).append('   ').append(second_A);
	return div;
}
    ...
    document.adminForm.submit();
      ...
      <div id="content">
        <div class="content_inner">            
            ...
            <?php $path="&searchwiki=".$wikistatus."&line=".$line."&match=".$match."&check=".$check."&error=".$error;?>
            <div class="table_nav">
            <form action='/content/import' method='post'>
              ...
              上下线:
              <select name='line'>
                <option value='-1' <?php if ($line=='-1') echo 'selected'; ?>>全部</option>
                <option value='1' <?php if ($line=='1') echo 'selected'; ?>>上线</option>
                <option value='0' <?php if ($line=='0') echo 'selected'; ?>>下线</option>
              </select>
              匹配:
              <select name='match'>
                <option value='0' <?php if ($match=='0') echo 'selected'; ?>>全部</option>
                <option value='1' <?php if ($match=='1') echo 'selected'; ?>>人工匹配</option>
                <option value='-1' <?php if ($match=='-1') echo 'selected'; ?>>自动匹配</option>
              </select>
              校对:
              <select name='check'>
                <option value='-1' <?php if ($check=='-1') echo 'selected'; ?>>全部</option>
                <option value='1' <?php if ($check=='1') echo 'selected'; ?>>已校对</option>
                <option value='0' <?php if ($check=='0') echo 'selected'; ?>>未校对</option>
              </select>
              错误:
              <select name='error'>
                <option value='-1' <?php if ($error=='-1') echo 'selected'; ?>>全部</option>
                <option value='1' <?php if ($error=='1') echo 'selected'; ?>>有错误</option>
                <option value='0' <?php if ($error=='0') echo 'selected'; ?>>无错误</option>
              </select>
              查询字段:
              <select name='searchtype'>
                ...
                <option value='3' <?php if ($typestatus=='3') echo 'selected'; ?>>subcontent_id</option>
              </select>
            ...
            <!--<a href='/content/import?deleted=1' style='margin-left:20px'><b style='color:red'>查看已删除</b></a>-->
                ...
                <div class="paginator">
                  <span class="first-page">
                      <a href="<?php echo url_for("content/import?page=".$imports->getFirstPage().$path);?>">
                      最前页
                      </a>
                  </span>
                  <span class="prev-page">
                     <a href="<?php echo url_for("content/import?page=".$imports->getPreviousPage().$path);?>">上一页</a>
                  </span>
                  <span class="pages">
                    ...
                    <?php foreach ($links as $key => $value):?>
                        ...
                        <?php else:?>
                            <a href="<?php echo url_for("content/import?page=".$value.$path );?>"><?php echo $value;?></a>
                        <?php endif;?>
                    <?php endforeach;?>
                  </span>
                  <span class="next-page"><a href="<?php echo url_for("content/import?page=".$imports->getNextPage().$path);?>">下一页</a></span>
                  <span class="last-page"><a href="<?php echo url_for("content/import?page=".$imports->getLastPage().$path);?>">最末页</a></span>
            ...
            <table cellspacing="0" id="admin_list" class="adminlist">
              <thead>
                <tr>
                  ...
                  <th scope="col" class="list_model" style="width: 6%;">类型</th>
                  <th scope="col" class="list_id">title/contentID</th> 
                  <th scope="col" class="list_id">导演/演员/年代</th>  
                  ...
                  <th scope="col" class="list_action" style="width: 6%;">操作</th>
                </tr>
              </thead>
              <tfoot>
                <tr>
                  <th scope="col"><input type="checkbox" id="sf_admin_list_batch_checkbox_foot" onclick="checkAll();"></th>
              ...
              <?php if(isset ($imports)):?>
                <?php
                    $types=array('series'=>'电视剧','program'=>'电影','column'=>'栏目');
                    $k=0; 
                    foreach ($imports->getResults() as $key => $import): 
                        $wiki=$import->getWiki();
                        $wikiStarring='';
                        $wikiDirector='';
                        $wikiTitle='';
                ...
                <tr edit="0">
                  ...
                  <td class="noEdit">
                  <?php if($import->getStateError()==1):?>
                    <font style="color:#ff0000"><?php echo $types[$import->getFromType()];?></font>
                  <?php else:?>
                    <?php echo $types[$import->getFromType()];?>
                  <?php endif;?>
                  <?php if($import->getStateCheck()==1):?><br/><font style="color:#0000ff">已校对</font><?php endif;?>
                  </td>
                  <td class="noEdit" name="contentId"><a target="_blank" href="<?php echo '/content/viewImport?id='.$import->getId();?>"><?php echo $import->getFromTitle();?><br /><?php echo $import->getFromId();?></a></td>
                  <td class="noEdit" name="title"><a title="<?php echo $injects[$k]['Description'];?>"><?php echo $injects[$k]['Director'];?><br /><?php echo $injects[$k]['Actors'];?><br /><?php echo $injects[$k]['Year'];?></a></td>
                  <td class="wiki" name="wiki" rel="<?php echo ($import->getWikiId()) ? $import->getWikiId() : ''; ?>"><?php echo $wikiTitle;?></td>
                  <td class="noEdit" name="title1"><a title="<?php echo $wikiContent;?>"><?php echo $wikiDirector;?><br /><?php echo $wikiStarring;?><br /><?php echo $released;?></a></td>
                  <td class="noEdit"><?php echo $import->getCreatedAt()->format("Y-m-d H:i:s");?></td>
                  <td class="noEdit"><?php echo ($updated_at = $import->getUpdatedAt()) ? $updated_at->format("Y-m-d H:i:s") : '';?></td>
