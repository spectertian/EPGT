...
function canel_(id,type,code){
    ...
    <div id="content">
        <div class="content_inner">
            ...
            <div class="table_nav">
              <?php include_partial('search', array('haveCode'=>$haveCode,'epg'=>$epg,'epgbak'=>$epgbak)); ?>
 <form action=""  method="post" name='adminForm' id='adminForm'>
            <div class="paginator">
              <span class="first-page">
                  <a href="<?php echo url_for('/spservice/index?page='.$pager->getFirstPage().'&name='.$name.'&type_='.$type_.'&haveCode='.$haveCode.'&epg='.$epg.'&epgbak='.$epgbak);?>">
                  最前页
                  </a>
              </span>
              <span class="prev-page">
                 <a href="<?php echo url_for('/spservice/index?page='.$pager->getPreviousPage().'&name='.$name.'&type_='.$type_.'&haveCode='.$haveCode.'&epg='.$epg.'&epgbak='.$epgbak);?>">上一页</a>
              </span>
              <span class="pages">
                  <?php foreach ($pager->getLinks(5) as $page ):?>
                        ...
                        <?php else:?>
                            <a href="<?php echo url_for('/spservice/index?page='.$page.'&name='.$name.'&type_='.$type_.'&haveCode='.$haveCode.'&epg='.$epg.'&epgbak='.$epgbak);?>"><?php echo $page;?></a>
                        <?php endif;?>
                  <?php endforeach;?>
              </span>
              <span class="next-page"><a href="<?php echo url_for('/spservice/index?page='.$pager->getNextPage().'&name='.$name.'&type_='.$type_.'&haveCode='.$haveCode.'&epg='.$epg.'&epgbak='.$epgbak);?>">下一页</a></span>
              <span class="last-page"><a href="<?php echo url_for('/spservice/index?page='.$pager->getLastPage().'&name='.$name.'&type_='.$type_.'&haveCode='.$haveCode.'&epg='.$epg.'&epgbak='.$epgbak);?>">最末页</a></span>
            ...
            <table cellspacing="0">
              <thead>
                <tr>
                  ...
                  <th scope="col" class="list_updated_at" style='width:21%'>channel_code/编码</th>
                  <th scope="col" class="list_updated_at" style='width:7%'>监测epg</th>
                  <th scope="col" class="list_updated_at" style='width:7%'>监测回看</th>
              ...
              <tfoot>
                <tr>
                  ...
                  <th scope="col" class="list_updated_at">channel_code/编码</th>
                  <th scope="col" class="list_updated_at">监测epg</th>
                  <th scope="col" class="list_updated_at">监测回看</th>
                ...
                <?php if(isset ($pager)):?>
                  ...
                  foreach ($pager->getResults() as $i => $rs):?>
                            <tr>
                              ...
                              <td>
                                <li style='width:150px' id='show<?php echo $rs->getId(); ?>' style='display: on' class='jc show' text='<?php echo $rs->getChannelCode(); ?>' val='<?php echo $rs->getId(); ?>'><?php if($rs->getChannelCode()){echo ($rs->getChannelCode());}else{echo '暂无';} ?><br /><?php echo $rs->getChannelNum();?></li>
                              ...
                              <td>
                                <?php $checkepg = $rs->getCheckEpg()?>
                                <?php if ($checkepg): ?>
                                  <a href="<?php echo "/spservice/checkEpg?id=".$rs->getId()?>"><?php echo image_tag('accept.png', array('alt' => __('Checked', array(), 'sf_admin'), 'title' => __('点击取消监测', array(), 'sf_admin'))) ?></a>
                                <?php else: ?>
                                   <a href="<?php echo "/spservice/checkEpg?id=".$rs->getId()?>"><?php echo image_tag('delete.png', array('alt' => __('Unhecked', array(), 'sf_admin'), 'title' => __('点击开启监测', array(), 'sf_admin'))) ?></a>
                                <?php endif; ?>
                              </td>
                              <td>
                                <?php $checkepgbak = $rs->getCheckEpgbak()?>
                                <?php if ($checkepgbak): ?>
                                  <a href="<?php echo "/spservice/checkEpgbak?id=".$rs->getId()?>"><?php echo image_tag('accept.png', array('alt' => __('Checked', array(), 'sf_admin'), 'title' => __('点击取消监测', array(), 'sf_admin'))) ?></a>
                                <?php else: ?>
                                   <a href="<?php echo "/spservice/checkEpgbak?id=".$rs->getId()?>"><?php echo image_tag('delete.png', array('alt' => __('Unhecked', array(), 'sf_admin'), 'title' => __('点击开启监测', array(), 'sf_admin'))) ?></a>
                                <?php endif; ?>                              
                              </td>
                              <td><?php echo ($updated_at = $rs->getUpdatedAt()) ? $updated_at->format("Y-m-d H:i:s") : '0000-00-00 00:00:00';?></td>
                              <td><li class='_status_' id='<?php echo $rs->getId(); ?>'><a href="<?php echo '/spservice/edit?id='.$rs->getId();?>" class="recommend">编辑</a> | <a href="<?php echo '/program?type='.$type.'&channel_code='.$rs->getChannelCode().'&date='.date('Y-m-d',time());?>">管理节目</a></li></td>
                            </tr>
                   <?php endforeach;?>
                <?php endif;?>
