...
$(document).ready(function(){
    ...
    $("#goToFilters").click(function(){
        ...
        var url = '<?php echo url_for($action) ?>' + '?date=' + date;
  ...
  <div class="widget-body">
    <ul class="week-action">
          <?php
            $tmps = array();
            $times = 24 * 60 * 60;
            $current_date = $sf_request->getParameter('date',date("Y-m-d",time())) ;
            ...
            for ($i = 0; $i <= 8; ++$i) {
                ...
                echo sprintf('<li><a href="%s" class="%s">%s</a></li>',
                        url_for($action).'?date=' . $tmps[$i],  $current_date == $tmps[$i] ? 'active' : '', (($i != 0 && $i != 8 ) ? $tmps[$i] : '') . $weeks[$i]);
            }
      ?>
