...
function textToActions(action,is_new){
	...
	if(action == 'save'){
		first_A.text(' 保存 ').attr({'href':'#'}).unbind('click').bind('click',function(){
                        ...
                        //提交至服务端处理                        
                        $.post('<?php echo url_for('programWeek/Save') ?>',program, function(data,textStatus){
...
function program_deleteajax(ids){
    $.ajax({
        url: '<?php echo url_for('programWeek/deleteAjax')?>',
        type: 'post',
        dataType: 'text',
        data: {'ids': ids},
        success: function(data){
            location.href='<?php echo url_for('programWeek/index')?>'; 
        },
    });
...
function program_delete(element,method){
        ...
        admin_form.action = "<?php echo url_for('programWeek/delete') ?>";
        admin_form.submit(); 
}
...
<table cellspacing="0" class="adminlist" id="admin_list">
  <thead>
    <tr>
      ...
      <th scope="col" class="time" name="time" style="width: 10%;">播放时间</th>
      <th scope="col" class="date" name="date" style="text-align: center;width: 15%">开始结束时间</th>
      <th scope="col" class="sort" name="sort" style="text-align: center;width:10%;">更新日期</th>
      <th scope="col" style="width:10%;text-align: center;" class="noEdit actions edit">操作</th>
    </tr>
  </thead>
  <tfoot>
    <tr>
     <th scope="col" class="list_checkbox"><input type="checkbox"  id="sf_admin_list_batch_checkbox_foot" onclick="checkAll(2);"></th>
      ...
      <th scope="col" style="text-align: center;">更新日期</th>
    ...
    <tr edit="0">
      ...
      <td class="sf_admin_text sf_admin_list_td_channel noEdit channel bbbb" name="channel_id" style="text-align:center;">
       ...
       <td class="sf_admin_date sf_admin_list_td_date start_time noEdit" name="date">
           <?php echo $program->getStartTime()->format("Y-m-d H:i") ?><br />
           <?php echo $program->getEndTime()->format("Y-m-d H:i") ?>
       </td>
       <td class="sf_admin_text sf_admin_list_td_updated_at noEdit updated_at" name="updated_at" style="text-align: center;">
           <?php echo $program->getUpdatedAt()? $program->getUpdatedAt()->format("Y-m-d H:i:s"):$program->getCreatedAt()->format("Y-m-d H:i:s")?>
       </td>
