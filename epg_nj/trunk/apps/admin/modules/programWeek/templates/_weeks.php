...
$(document).ready(function(){
    ...
    $("#goToFilters").click(function(){
        ...
        var url = '<?php echo url_for('programWeek/index') ?>' + '?type=<?php echo $type; ?>&channel_code=<?php echo $sf_user->getAttribute('channel_code') ?>' +  '&date=' + date;
          ...
          <?php
            ...
            for ($i = 0; $i <= 8; ++$i) {
                ...
                echo sprintf('<li><a href="%s" class="%s" link=\'{"channel_c":"%s","date":"%s"}\'>%s</a></li>',
                        url_for('programWeek/index').'?type='.$type.'&channel_code=' . $channel_code . '&date=' . $tmps[$i],  $current_date == $tmps[$i] ? 'active' : '', $channel_code, $tmps[$i], (($i != 0 && $i != 8 ) ? $tmps[$i] : '') . $weeks[$i]);
            }
      ?>
