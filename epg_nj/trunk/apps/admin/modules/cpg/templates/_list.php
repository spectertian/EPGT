	...
	if(action == 'save'){
		first_A.text(' 保存 ').attr({'href':'#'}).unbind('click').bind('click',function(){
                        ...
                        */                      
                        ...
                        //提交至服务端处理                        
                        $.post('<?php echo url_for('cpg/Save') ?>',program, function(data,textStatus){
...
function changeTR(element,new_tr_label,is_new){
    ...
    var edit = tr.attr("edit",1); //设置TR为编辑状态
        ...
        if(_element.hasClass('wiki')){
            input.simpleAutoComplete('<?php echo url_for('cpg/loadWiki') ?>',{
...
function program_deleteajax(ids){
    $.ajax({
        url: '<?php echo url_for('cpg/deleteAjax')?>',
        type: 'post',
        dataType: 'text',
        data: {'ids': ids},
        success: function(data){
            //alert(data);
            location.href='<?php echo url_for('cpg/index')?>'; 
        },
    });
...
function program_delete(element,method){
        ...
        admin_form.action = "<?php echo url_for('cpg/delete') ?>";
        admin_form.submit(); 
}
...
function Publish(publish)
{
    ...
    admin_form = document.getElementById('adminForm');
    admin_form.action = "<?php echo url_for('cpg/publish')?>";
    admin_form.submit();
}
...
<table cellspacing="0" class="adminlist" id="admin_list">
  <thead>
    <tr>
      <th scope="col" class="noEdit checkbox"><input id="sf_admin_list_batch_checkbox" name="toggle" onclick="checkAll(1);" type="checkbox" name="ids" style="width:5%"></th>
      <th scope="col" class="list_id" name="name" style="width: 15%">名称</th>
      <th scope="col" class="title sf_admin_text sf_admin_list_th_channel noEdit channel" name="channel_id" style="width: 10%">频道</th>
      <th scope="col" class="time" name="time" style="width: 8%;">播放时间</th>
      <!--<th scope="col" class="wiki" name="wiki" style="width: 10%">维基</th>-->
      <!--<th scope="col" class="tags" name="tags" style="width: 7%">Tags</th>-->
      <th scope="col" class="date" name="date" style="text-align: center;" style="width: 10%">创建日期</th>
      <th scope="col" class="sort" name="content_id" style="text-align: center;" style="width:25%;">asset_id</th>
      <!--<th scope="col" style="width:7%;text-align: center;" class="noEdit actions edit" style="width:10%;">操作</th>-->
    </tr>
  </thead>
  <tfoot>
    <tr>
     <th scope="col" class="list_checkbox"><input type="checkbox"  id="sf_admin_list_batch_checkbox_foot" onclick="checkAll(2);"></th>
      ...
      <th scope="col" style="text-align: center;">content_id</th>
  ...
  ?>
    <tr edit="0">
      ...
      <td class="sf_admin_text sf_admin_list_td_name" name="name" style="text-align:left;" >
          <?php echo $program->getProgramName() ?>
      </td>
      <td class="sf_admin_text sf_admin_list_td_channel noEdit channel bbbb" name="channel_id" style="text-align:center;">
        ...
        <td class="sf_admin_date sf_admin_list_td_time time" name="time">
          <?php echo date('H:i',$program->getStartTime()->getTimestamp()) ?>
        </td>
       ...
       <td class="sf_admin_text sf_admin_list_td_sort content_id" name="content_id" style="text-align: center;">
           <?php echo $program->getContentId() ?>
       </td>
