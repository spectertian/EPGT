...
{
    admin_form = document.getElementById('adminForm');
    admin_form.action = "<?php echo url_for('reportchannel/batchUnState')?>";
    admin_form.submit();
}
                ...
                <?php if(isset ($pager)):?>
                  <?php foreach ($pager->getResults() as $i => $rs): ?>
                            <tr>
                              ...
                              <td>
                                    <?php if ($rs->getState()): ?>
                                      <a href="<?php echo url_for("reportchannel/unstate?id=".$rs->getId())?>"><?php echo image_tag('accept.png', array('alt' => __('Unhecked', array(), 'sf_admin'), 'title' => __('已处理：点击处理', array(), 'sf_admin'))) ?></a><?php echo $rs->getUser();?>
                                    <?php else: ?>
