...
<!--          <li>地区: <strong><span class="no-warp"><?php //echo trim($wiki->getArea()); ?></span></strong></li>-->
      ...
      <div class="right-col">
        ...
        <div class="stills">
              <ul>
              <?php
                $dramatis = $wiki->getScreenshotAll();
                $photo_count = count($dramatis) -1 ;
                for($i=0;$i<=3;$i++):
                if(($photo_count >= $i) && ($photo_count > 0) ):
              ?>
                  <li <?php echo ( $i == 3 ) ? 'class="last"' : 'class=""' ?>><img src="<?php echo file_url($dramatis[$i]['wiki_value']) ?>" /></li>
              <?php else: ?>
