...
//					$wiki = $wikiRepository->findOneById(new MongoId($wikiRec['wiki_id']));
     ...
     */
     public function GetRecommendMedias($param) 
     {
        ...
        $memcache_key = 'GetRecommendMedias'.",$size,$sort,$detail,$userId";	//Modify by 缓存的key不经过md5加密 tianzhongsheng-ex@huan.tv Time 2013-04-22 18:21:00
    ...
    public function SearchWiki(array $parameter) {
        ...
        $offset = $size * ($page - 1);
        $total = NULL;
        $hasvide = $parameter['hasvideo'] == "true" ? true : false;
        $mongo = sfContext::getInstance()->getMondongo();
        $wikiRep = $mongo->getRepository('wiki');
        if($hasvide){
            $result = $wikiRep->xun_search("hasvideo:1 $keyWord", $total, (int)$offset, (int)$size,null,1);
        }else{
