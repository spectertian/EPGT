 ...
 * @author     Masao Maeda <brt.river@gmail.com>
  */
class sfMongoSessionStorage extends sfDatabaseSessionStorage
{    
    protected $host = null, $port = null, $db = null, $col = null;
    ...
    {    
        ...
        if (!empty($_POST[$sessionName])) {
            $_COOKIE[$sessionName] = $_POST[$sessionName];
        } elseif (!empty($_GET[$sessionName])) {
            $_COOKIE[$sessionName] = $_GET[$sessionName];
        } elseif (!empty($_COOKIE[$sessionName])) {
            $_COOKIE[$sessionName] = $_COOKIE[$sessionName];
        }
    ...
    public function sessionOpen($path = null, $name = null)
    {
        ...
        if($databaseClass == 'sfMondongoDatabase')
        {
            $this->db = $database->getMondongoConnection()->getMongoDB();
            $this->con = $database->getMondongoConnection()->getMongoDB();
            $this->col = $this->db->selectCollection($this->options['db_table']);
        }
...
//  }
  ...
  public function sessionRead($id)
  {
    ...
    $obj = $this->col->findOne(array($db_id_col => $id));
    if (count($obj))
    {
      // found the session
    ...
    {
      //由于数据库采用的模式为 CAPPED,所以导致第N次更新时的字段内容不能大于第N-1次的字段内容.故第一次初始化时插入2000个无效字符
      $data = str_pad(' ',2000);
      $obj = array(
        $db_id_col => $id,
        $db_data_col => $data,
