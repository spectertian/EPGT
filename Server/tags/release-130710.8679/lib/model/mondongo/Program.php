...
class Program extends \BaseProgram
{
    ...
    public function getWiki() {
        //if (!isset($this->wiki)) {//注释此，防止wiki修改后program无反应的问题
...
//            $xapian_document->add_term('Z'.$word['word']);
    ...
    public function postSave() {
		...
		$ps = $psRepos->findOne(array('query'=>array('program_id'=>(string)$id)));
		if ($ps) 
			$ps->delete();
		//获取关键字
		...
		if($values)
		{
			...
			if($arr)
			{
				foreach($arr as $k)
				{
					if(preg_match($k,$wiki_title))
					{
						$this->saveProgramSport();
						return false;
					}
					if(preg_match($k,$tags))
					{
						$this->saveProgramSport();
						return false;		
					}				
	...
	public function saveProgramSport()
	{
    	$ps = new ProgramSport();
    	$ps->setProgramId  ((string)$this->getId());
    	$ps->setName       ($this->getName()       ?$this->getName()       :'');
    	$ps->setPublish    ($this->getPublish()    ?$this->getPublish()    :'');
    	$ps->setChannelCode($this->getChannelCode()?$this->getChannelCode():'');
    	$ps->setTags       ($this->getTags()       ?$this->getTags()       :'');
    	$ps->setStartTime  ($this->getStartTime()  ?$this->getStartTime()  :'');
    	$ps->setEndTime    ($this->getEndTime()    ?$this->getEndTime()    :'');
    	$ps->setTime       ($this->getTime()       ?$this->getTime()       :'');
    	$ps->setDate       ($this->getDate()       ?$this->getDate()       :'');
    	$ps->setWikiId     ($this->getWikiId()     ?$this->getWikiId()     :'');
    	$ps->setAdmin      ($this->getAdmin()      ?$this->getAdmin()      :'');
    	$ps->setSort       ($this->getSort()       ?$this->getSort()       :'');
    	$ps->setTvsouId    ($this->getTvsouId()    ?$this->getTvsouId()    :'');				
    	$ps->save();		
	}
        /**
     * 视频删除事件
