...
//        $wikis = $this->find(array("query" => array("_id" => array('$in' => $wiki_ids))));
    ...
    public function getWikiBySlug($wikiSlug){
        $memcache = tvCache::getInstance();
        $memcache_key = $wikiSlug;	//Modify by 缓存的key不经过md5加密 tianzhongsheng-ex@huan.tv Time 2013-04-22 18:11:00
    ...
    public function getWikiByTitle($wikiSlug){
        $memcache = tvCache::getInstance();
        $memcache_key = 'getWikiByTitle'.$wikiSlug;	//Modify by 缓存的key不经过md5加密 tianzhongsheng-ex@huan.tv Time 2013-04-22 18:12:00
        $wiki = $memcache->get($memcache_key);
