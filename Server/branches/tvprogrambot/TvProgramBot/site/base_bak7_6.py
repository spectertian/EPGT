...
class SiteBase(object):
    ...
    def save(self, programs):
        ...
        if programs:
            ...
            for program in programs:
                ...
                wiki = self.get_wiki_from_edit_memory(p['name'])
                if not wiki:
                    wiki = self.get_wiki_from_time(p['name'], p['date'], p['time'])
                if wiki:
                    p['wiki_id'] = str(wiki['_id'])
                    #p['tags'] = wiki['tags']
                elif program['wiki'].has_key("tvsou_id"):
                    ...
                    if wiki:
                        ...
                        if len(wiki_title) > 1:
                            p['wiki_id'] = str(wiki['_id'])
                            #p['tags'] = wiki['tags']
                has_program = self.mongo_program.find_one({"channel_code": self.channel_code, "date": p['date'], "time": p['time']})
                # 已存在节目表不做变动，防止覆盖编辑已做之修改
                if has_program:
                    if p['name'] != has_program['name'] and not has_program.get("admin"):
                        has_program['name'] = p['name']
                        if p.has_key("wiki_id") and p['wiki_id'] in skip_wiki_id:
                            del p['wiki_id']
                            del p['tags']
                        elif p.has_key("wiki_id"):
                           has_program["wiki_id"] = p['wiki_id']
                           has_program["tags"] = p['tags']
                        self.mongo_program.save(has_program)
                    continue
                # 跳过严重错误之维基
                if p.has_key("wiki_id") and p['wiki_id'] in skip_wiki_id:
                    del p['wiki_id']
    ...
    def get_wiki_from_edit_memory(self, program_name):
        """从编辑的编辑记录库中查询wiki"""
        editmemory = self.mongo_editmemory.find_one({"program_name": program_name})
        if editmemory:
