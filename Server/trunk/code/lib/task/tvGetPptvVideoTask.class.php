...
class tvpptvVideoTask extends sfMondongoTask
{
	...
	protected function execute($arguments = array(), $options = array())
	{
		...
		$xmlcontent = Common::get_url_content("http://epg.api.pptv.com/cover_image_recommand_nav-ex.api?auth=2&mode=1&platform=ipad");
		...
		foreach($xmlobj->nav as $a => $xmlnav) {
			...
			$xmlcontent = Common::get_url_content("http://epg.api.pptv.com/cover_image_recommand_list-ex.api?auth=2&nav_id=".$xmlnav->navid);
			$xmlobj = simplexml_load_string($xmlcontent);
	...
	{
		$xmlcontent = Common::get_url_content("http://epg.api.pptv.com/detail.ashx?platform=stb&c=28&s=1&vid=".$vid."&auth=1");
		$xmlobj = simplexml_load_string($xmlcontent);
