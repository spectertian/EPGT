...
{	
    ...
    {
    	//	by tianzhongsheng memcache判断  后台节目表不需要memcache 2013-08-01 11：28：00 
        return $this->getDayProgramsByChannelCode($channel_code, $date, $ifwiki,$is_memcache = true);
    }
     ...
     * @describe 添加了分页
     */
    public function getDayProgramsByChannelCode($channel_code, $date, $ifwiki = false,$is_memcache = false, $page = 1, $pagesize = 100) 
    {   
        $memcache = tvCache::getInstance();
        $memcache_key = "Program_getDays_".$channel_code."_".$date."_".$ifwiki."_".$page."_".$pagesize;
        $programs = $memcache->get($memcache_key);
        $skip = ($page - 1) * $pagesize;
        if(!$programs || $is_memcache) {	//	by tianzhongsheng memcache判断  后台节目表不需要memcache 2013-08-01 11：28：00 
            $query = array(
            ...
            $memcache->set($memcache_key, $programs, 600);
        }
        return $programs;
    }
     ...
     * @modified zhigang 2011-1-4
     */
    public function getCityLiveProgramByTag($city, $province, $tag, $limit = 50) 
    {
        if(!$province) {
            $search_type = array("cctv","tv"); 
        } else {
            $search_type = array("cctv","tv",$province);
        }    
        
        $now = new MongoDate();
        $query = array(
	        'channel_type' => array('$in' => $search_type),
     ...
     * @author lifucang
     */    
    public function getLivePrograms($channel_codes, $now = '', $has_wiki_id = true ,$page = 1, $pagesize = 100) 
    {
	    ...
	    return $this->find(array('query' => $query, 'sort'=>array('time' => -1),'skip'=>$skip,'limit'=>$pagesize,));		
    }
    ...
    /**
     * 根据 tag,starttime,endtime 获取某城市的节目列表
     * @param <string> $city 节目所属频道code
     * @param <string> $province 节目所属频道code
     * @param <type> $tag 节目标识
     * @param <type> $starttime 节目开始时间
     * @param <type> $endtime   节目结束
     * @return <type>
     ...
     * @editor lifucang  2012-6-18  增加排序字段sort
     */
    public function getCityPrograms($city, $province = '', $tag = '', $starttime, $endtime, $page = 1, $pagesize = 100) 
    {
    	$starttime = empty($starttime) ? new MongoDate(time()) : new MongoDate(strtotime($starttime));
    	$endtime   = empty($endtime)   ? new MongoDate(mktime(23, 59, 0, date('m'), date('d'), date('Y'))) : new MongoDate(strtotime($endtime));
    	$skip = ($page - 1) * $pagesize;
     ...
     * @author wangan
     */
    public function getNextUpdateByCity($city, $province = '' , $endtime = '', $tag = '')
    {  
		if(empty($endtime)) {
			$endtime = date("Y-m-d H:i:s",mktime(23, 59, 0, date('m'), date('d'), date('Y')));
        }
     ...
     * @author luren
     */
    public function searchCityPrograms($city,$province,$key='',$starttime,$endtime) 
    {	
    	$starttime   = empty($starttime)?time():strtotime($starttime);
    	$m_starttime = new MongoDate($starttime);
    ...
    /**
     * 自动为相同program匹配WIKI，当前时间以后本周之前的节目
     * 
     * @author gaobo
     */
    public function autoWikiByTvsouid($time,$channel_code,$wiki_id,$tvsouid)
    {
        ...
        /* 
         * @desc 只匹配明天节目的维基 
         * @auther tianzhongsheng-ex 
         * @time 2013-08-02 15:30
        $unix = strtotime($time);
        $now = new MongoDate($unix);
        */
		$date = date("Y-m-d",strtotime("+1 day"));
        $programs = $this->find(
                array(
                        'query' => array(
                                "tvsou_id" => strval($tvsouid),
                                "channel_code" => $channel_code,
                				"date" => $date,
//                                'start_time' => array('$gt' => $now),
     ...
     * 
     * 此方法为名称正则匹配，节目匹配WIKI暂时弃用此方法 ,并启用新方法为autoWikiByTvsouid   the comment edit by gaobo at 2013-07-11
     */ 
    public function autoWiki($time,$channel_code,$wiki_id,$program_name)
    {
        $mongo = $this->getMondongo();
		$repository = $mongo->getRepository('wiki');         
    	$wiki = $repository->findOneById(new mongoId($wiki_id));
        ...
        */
        ...
        $patterns = array('/\(/','/\)/','/\d/');
        $repatt = array('\(','\)','\d+');
        $program_name = preg_replace($patterns, $repatt, $program_name);
        */
        $program_name = preg_replace("/\(/",'\(',$program_name);
        $program_name = preg_replace("/\)/",'\)',$program_name);
        $program_name = preg_replace("/\d/",'\d+',$program_name);
	    $programs = $this->find(
	                    array(
	                        'query' => array(
	                            "name" => new MongoRegex("/$program_name/i"),
	                    		"channel_code" => $channel_code,
	                            'start_time' => array('$gt' => $now),
	                            //'end_time' => array('$lt' => new MongoDate(strtotime($end_date." 23:59:59 +".$i."day")))
	                            )
...
/*    	$starttime=empty($starttime)?new MongoDate(time()+(8*3600)):new MongoDate(strtotime($starttime)+(8*3600));//对的
     ...
     * @editor lifucang  2012-6-18  增加排序字段sort
     */
    public function getCityStartPrograms($city, $province = '', $tag = '',$starttime,$endtime, $page = 1, $pagesize = 100) 
    {	    	
    	$starttime = empty($starttime) ? new MongoDate(time())                                             : new MongoDate(strtotime($starttime));//对的
    	$endtime   = empty($endtime)   ? new MongoDate(mktime(23, 59, 0, date('m'), date('d'), date('Y'))) : new MongoDate(strtotime($endtime));
    	$skip = ($page - 1) * $pagesize;
     ...
     * @author qhm
     */
    public function getLiveProgramsWillPlay($now='',$page = 1, $pagesize = 100) {
    	$now = empty($now)?  new MongoDate():new MongoDate(strtotime($now));
    	$date_end = new MongoDate(mktime(23, 59, 0, date('m'), date('d'), date('Y')));
    	$skip = ($page - 1) * $pagesize;
    	return $this->find(
    			array(
    					'query' => array(
    							...
    							'start_time' => array('$gte' => $now, '$lte' => $date_end),
    							'wiki_id'=>array('$exists'=>true)
    					),
    					'sort'=>array('start_time' => 1),
    					'skip'=>$skip,
     ...
     * @author qhm
     */
    public function getDayProgramsByChannel($date, $ifwiki = false,$is_memcache = false, $page = 1, $pagesize = 100)
    {
    	$memcache = tvCache::getInstance();
        $memcache_key = "Program_getDays_Channellist_".$date."_".$ifwiki;
    	$programs = $memcache->get($memcache_key);
    	$skip = ($page - 1) * $pagesize;
    	//if(!$programs || $is_memcache) {
    		$query = array(
    				'date' => $date
    		);
    		if($ifwiki) {
    		...
    		//$memcache->set($memcache_key, $programs, 6000);
      //	}
     ...
     * @author luren
     */
    public function searchPrograms($key='',$starttime,$endtime, $page = 1, $pagesize = 100)
    {
    	$starttime   = empty($starttime)?time():strtotime($starttime);
    	$m_starttime = new MongoDate($starttime);
    	$m_endtime   = empty($endtime)?new MongoDate(($starttime + 3600*2)):new MongoDate(strtotime($endtime));
     ...
     * @author jhm
     */
    public function getdayNotPlayedProgramByWikiId($wiki_id)
    {
    	$date_start = new MongoDate(time());
    	$date_end = new MongoDate(mktime(23, 59, 0, date('m'), date('d'), date('Y')));
    	$rets = $this->find(
    			array(
    					'query' => array(
