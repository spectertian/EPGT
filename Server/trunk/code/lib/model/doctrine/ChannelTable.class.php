...
class ChannelTable extends Doctrine_Table
{
    ...
    public static function setStatus($id, $is_show = 1)
    {
        if(is_array($id)){
            foreach($id as $idid){
                self::ajaxUpdateBatch($idid, "publish", $is_show);
            }
            return array('code'=>1, 'msg'=>'更新成功');
        }
        return self::ajaxUpdate($id, "publish", $is_show);
    }
     ...
     * @gaobo 
     */
    public static function ajaxUpdateBatch($id, $name, $value)
    {
        if(!is_numeric($id)) {
    ...
    */
    ...
    {
        $memcache = tvCache::getInstance();
        $memcache_key = 'Channel_getAllList';
        $channels = $memcache->get($memcache_key);
        if(!$channels) {
            $channels = $this->createQuery()
                        ->Where('publish = ?', 1)
    ...
    */
    ...
    /**
     * 根据 名称 取得已发布 channel 列表
     * @author lifucang
     * @date  2013-11-11
     */
    public function getChannlesByNames($name) 
    {
        $memcache = tvCache::getInstance();
        $memcache_key = 'Channel_getChannlesByNames_'.$name;
        $channels = $memcache->get($memcache_key);
        if(!$channels){ 
            $channels = $this->createQuery()
                ->where('name = ?', $name)
