...
过滤:电视台:
    ...
    <?php endif?>
     ...
     <?php $date=$sf_request->getParameter('date')? $sf_request->getParameter('date') : date("Y-m-d");?>
     <a href="<?php echo url_for('program/SetEndTime?channel_code='.$channel_code.'&date='.$date) ?>">重新生成结束时间</a> 
     </br></br>
     TVSOU更新时间：<?php if($updatetime){echo $updatetime;}else{echo "<font color='red'>今天无更新</font>";} ?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 编辑确认时间：<?php if($et){echo $et;} else {echo "<font color='red'>还没有编辑进行确认</font>";} ?>
</form>
