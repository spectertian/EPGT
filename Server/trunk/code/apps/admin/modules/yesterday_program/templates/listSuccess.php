<script type="text/javascript">
	...
	<div class="r">
		<header>
			...
			<nav class="utility">
				<li class="add"><a href="<?php echo "/yesterday_program/add?date=".$date;?>">添加</a></li>
				<li class="save"><a href="#" onclick="save($(this));">另存为</a></li>
				<li class="back"><a href="<?php echo "/yesterday_program?date=".$date ?>">返回列表</a></li>
			</nav>
		</header>
		<?php include_partial('weeks'); ?>
		<div id="stock">
		<?php if($yesterDayPrograms):?>
		<div class="listwarp" >
			<ul class="list" >
			<?php foreach($yesterDayPrograms as $k => $rs):?>
					...
					<li class="mvcover">
							...
							<a href="<?php echo "/yesterday_program/edit?id=".$rs->getId()?>" class="edit">编辑</a>|
							<a href="<?php echo "/yesterday_program/delete?id=".$rs->getId()?>" onclick="if(!confirm('确定删除吗？')) return false;">删除</a>
					</li>
	...
	<form name="form_1" method="post" id="form_1" action="/yesterday_program/OtherSave">
		<ul class="aboutsave">
			<li><h2>节目另存为</h2></li>
