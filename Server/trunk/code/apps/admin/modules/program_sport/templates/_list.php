	...
	if(action == 'save'){
		first_A.text(' 保存 ').attr({'href':'#'}).unbind('click').bind('click',function(){
                        ...
                        */                      
                        ...
                        //提交至服务端处理                        
                        $.post('<?php echo url_for('program_sport/Save') ?>',program, function(data,textStatus){
...
function changeTR(element,new_tr_label,is_new,front){
    ...
    var edit = tr.attr("edit",1); //设置TR为编辑状态
        ...
        if(_element.hasClass('noEdit')){                            //处理不需要生成文本框的元素
            ...
            if(_element.hasClass('channel_id') && is_new){
                var textNode = document.createTextNode('');
                td = insertElement(td,textNode,is_new,classes);
            }                       
        ...
        if(_element.hasClass('wiki')){
            input.simpleAutoComplete('<?php echo url_for('program_sport/loadWiki') ?>',{
...
//JSON POST 此数据由makeUpDate函数使用
var program = { 'id': 0 ,
                'name' : '',
               // 'channel_id' : <?php echo $sf_user->getAttribute('channel_id') ?>,//edit wn 此处去掉由 class="sf_admin_date sf_admin_list_td_channel_id channel_id"处提供 缘由 列出所有频道节目时 供一键保存使用
                'publish' : '',
                'time' : '',
                'date' : '',
                'wiki_id': '',
...
function program_deleteajax(ids){
    $.ajax({
        url: '<?php echo url_for('program_sport/deleteAjax')?>',
        type: 'post',
        dataType: 'text',
        data: {'ids': ids},
        success: function(data){
            //alert(data);
            location.href='<?php echo url_for('program_sport/index')?>'; 
        },
    });
...
function program_delete(element,method){
        ...
        admin_form.action = "<?php echo url_for('program_sport/delete') ?>";
        admin_form.submit(); 
}
...
function Publish(publish)
{
    ...
    admin_form = document.getElementById('adminForm');
    admin_form.action = "<?php echo url_for('program_sport/publish')?>";
    admin_form.submit();
}
    ...
    text-align:center;
            ...
            <div class="paginator">
              ...
              <?php if($pager->getPage()!=1): ?>
                  <a href="<?php echo url_for("program_sport/index?date=$date&page=".$pager->getFirstPage());?>">
                  最前页
                  </a>
              <?php endif; ?>
              </span>
              <span class="prev-page">
                 <a href="<?php echo url_for("program_sport/index?date=$date&page=".$pager->getPreviousPage());?>">上一页</a>
              </span>
              <span class="pages">
                ...
                <?php foreach ($links as $key => $value):?>
                    ...
                    <?php else:?>
                        <a href="<?php echo url_for("program_sport/index?date=$date&page=".$value );?>"><?php echo $value;?></a>
                    <?php endif;?>
                <?php endforeach;?>
              </span>
              <span class="next-page"><a href="<?php echo url_for("program_sport/index?date=$date&page=".$pager->getNextPage());?>">下一页</a></span>
              <?php if($pager->getLastPage()!=$pager->getPage()): ?>
                  <span class="last-page"><a href="<?php echo url_for("program_sport/index?date=$date&page=".$pager->getLastPage());?>">最末页</a></span>
              <?php endif; ?>
...
<table cellspacing="0" class="adminlist" id="admin_list">
  <thead>
    <tr>
      ...
      <th scope="col" class="date" name="date" style="text-align: center;">播放日期</th>
  ...
  <tbody>
  <?php if($pager):?>
    		<?php foreach ($pager->getResults() as $key => $program):?>
    <tr edit="0">
      ...
      <td class="sf_admin_text sf_admin_list_td_channel noEdit channel" name="channel_id" style="text-align:center;">
       ...
       <td class="sf_admin_date sf_admin_list_td_date date" name="date">
           <?php echo $program->getDate() ?>
       </td>
       <td class="sf_admin_date sf_admin_list_td_channel_id channel_id" style="display:none "name="channel_id">
           <?php echo $program->getChannel()->getId() ?>
       </td>
      ...
      <td class="sf_admin_text sf_admin_list_td_channel noEdit channel"  style="text-align:left;" >
        ...
        <td class="sf_admin_text sf_admin_list_td_actions noEdit actions edit" rel="<?php echo $program->getId();?>" style="text-align: center;">
            <?php if($program->getWikiId()):?><a target="_blank" href="<?php echo url_for("wiki/edit?id=".$program->getWikiId())?>">查看维基</a> | <?php endif;?><a href="#" onclick="changeTR($(this),'newProgram',false);return false;" >编辑</a> | <a href="#" onclick="if(confirm('确定删除吗？')){program_deleteone($(this),1);return false;}"  >删除</a>
        </td>
    </tr>
  <?php endforeach; ?>
<?php endif ?>
