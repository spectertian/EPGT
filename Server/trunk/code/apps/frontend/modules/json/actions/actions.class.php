...
class jsonActions extends sfActions
{
	...
	public function __construct($context=null, $moduleName=null, $actionName=null)
    {
		...
		$this -> action_array = array(                           
                            ...
                            'GetAttachments',				//广电专用,导出数据用 @gaobo 2012-12-12
                            'GetChannelsUpdate',			//广电专用,获取更新的频道
                            'GetContentImport',             //广电专用,获取匹配wiki后的ADI数据
                            'GetWikisDel',
                            'GetChannelsByNames',  
    ...
    */
    public function executeIndex(sfWebRequest $request) 
	{
        ...
        if ($request->getMethod() == 'POST'||$request->getMethod() == 'GET'||$request->isXmlHttpRequest()) {
			if($request->getParameter('jsonstr')){
			...
			$this->checkDeveloper($jsonstr["developer"]);
			if(is_array($this->developer)){
				$nodeArray = $this->getErrArray($this->developer['num'],$this->developer['notice']);
				return $this->renderText(json_encode($nodeArray));
			}
     ...
     * @final 2012-07-19
     */
    public function getErrArray($errorStatus=0,$message='',$total=null,$num=null,$page=null,$pagesize=null){
        $nodeArray = array();
	 ...
	 * @return boolean
	 */
	private function checkDeveloper($developer) {
		if (!$developer['apikey']||!$developer['secretkey']||!$developer) {
			$this->developer = array('num'=>'1005','notice'=>'请填写开发者信息');
			return ;
		}
		//修改cache和xml2公用一个缓存。 2013-7-24 majun
		...
		if(!$cacheDeveloper) {
            $this->developer = array('num'=>'1006','notice'=>'未知开发者');
			return ;
        }
		if($cacheDeveloper->getSecretkey() != $developer['secretkey']) {
            $this->developer = array('num'=>'1008','notice'=>'安全密钥不正确');
			return ;
		}else if($cacheDeveloper->getState() == '0'){
            $this->developer = array('num'=>'1007','notice'=>'该开发者已锁定');
			return ;
        }else if ($cacheDeveloper->getSecretkey() == $developer['secretkey']){
        	$this->developer = '1';
		    return ;
        }
		
		/*$visitNum = tvCache::getInstance()->get("secretkey_".$developer['apikey']);
		if ($visitNum){	
			    $state= tvCache::getInstance()->get("state_".$developer['apikey']);
				if ($state && $state=='0') {
					$this->developer = array('num'=>'1007','notice'=>'该开发者已锁定');
					return ;
				}
				if ($visitNum == $developer['secretkey']){
					$this->developer = '1';
					return ;
				}else {
					$this->developer = array('num'=>'1008','notice'=>'安全密钥不正确');
			...
			$mongo = $this->getMondongo();
			$repository = $mongo->getRepository('Developer');
			$dev = $repository->findOne(array('query' => array('apikey'=> $developer['apikey'])));
		    if ($dev){
				   tvCache::getInstance()->set("state_".$developer['apikey'],$dev->getState(),time() +  60*60*24*180); //状态，是否锁定
		       tvCache::getInstance()->set("secretkey_".$developer['apikey'],$dev->getSecretkey(),time() + 60*60*24*180); //安全密钥不正确 
		     	if ($dev->getState()=='0') {
		    		$this->developer = array('num'=>'1007','notice'=>'该开发者已锁定');
		    		return ;
		    	}
		    	if ($dev->getSecretkey() == $developer['secretkey']){
		    		$this->developer = '1';
		    		return ;
		    	}else {
		    		$this->developer = array('num'=>'1008','notice'=>'安全密钥不正确');
        ...
        if($channel)
        {
	        ...
	        foreach($programs as $key =>$program)
	        {
				...
				$nodeArray['program'][$key] = array(
					'name' => $program['name'],
					'date' => $program['date'],
					'start_time' => date("Y-m-d H:i",$program['start_time']->getTimestamp()),
					'end_time' => date("Y-m-d H:i",$program['end_time']->getTimestamp()),
                    'time' => $program['time'],
					'wiki_id' => $program['wiki_id'],
					'wiki_cover' => file_url($wiki_info['cover']),
					'tags' => $wiki_info['tags'],
	...
	/**
     * 根据SP或省份获取直播列表
     * @param Array(type,typevalue,tag,start_time,end_time) $param['typevalue']当$param['type']==1时为sp的名称，为2是省的名称
     * @return json
     * @author lifucang
     */    
	private function GetLiveProgrameByTag($param)
	{
		$nodeArray = array();
        $type=$param['type'] ? $param['type'] : 1;
        $page=$param['page'] ? $param['page'] : 1; //第几页
        $pagesize=$param['pagesize'] ? $param['pagesize'] : 10;	//每页的数量
        $typevalue=$param['typevalue'];
    	$tag = $param['tag'];
    	$starttime = $param['start_time'];
    	$endtime = $param['end_time'];
    	if($starttime == '' && $endtime == ''){
            $starttime = $endtime = date("Y-m-d H:i:s", time());
        }        
        if($type==1){
            if($typevalue==''||empty($typevalue)){
                $channels = Doctrine::getTable('Channel')->getWeiShiChannels();
		...
		$programs = $ProgramRepository->getPrograms($channels,$tag,$starttime,$endtime);
        unset($channels);
        $nodeArray = $this->getErrArray(0,'',count($programs),null,$page,$pagesize);
...
//			$wiki = $WikiRepository->findOneById(new MongoId($program['wiki_id']));
			...
			if($wiki) 
			{
				...
				$nodeArray['media'][$i] = array(
					...
					'channel_memo'=>$channel->getMemo(),	//modify by tianzhonsheng-ex 增加memo字段 2013-10-21 17：30：00
...
//				去掉视频源 tianzhongsheng-ex 2013-09-04 18:06			
//				$nodeArray = $this->getWikiVideoSource($wiki, $i, $nodeArray);
		...
		//返回的数组进行分页  modify by tianzhongsheng-ex@huan.tv 2013-10-22 10:07
		$pages = ceil($i/$pagesize);
		$page = $page>$pages?$pages:$page;
		$nodeArray['media'] = array_slice($nodeArray['media'],($page-1)*$pagesize,$pagesize);
...
//				$nodeArray = $this->getWikiVideoSource($wiki, $i, $nodeArray);
    ...
    public function GetMediaCategory($param){
        ...
        unset($nodeArray['total']);//getErrArray为公共方法，此处去掉total为暂时办法
        return $nodeArray;
    }
    ...
    public  function GetMediasByCategory($param){
        ...
        $cid = $param['cid'];
        $hasvideo = $param['need_video'] == "false" ? false : true ;
        ...
        switch($order)
        {
        	case '1':
				...
				break;
        	case '2':
                $nodeArray = $this->getMediaBySort($cid,$filters,$page,$size,0,$hasvideo);
				break;
        	case '3':
				$nodeArray = $this->getMediaBySort($cid,$filters,$page,$size,2,$hasvideo);
				break;
        	default:
...
//					$wiki = $wikiRepository->findOneById(new MongoId($wikiRec['wiki_id']));
     ...
     */
     public function GetRecommendMedias($param) 
     {
        ...
        $memcache_key = 'GetRecommendMedias'.",$size,$sort,$detail,$userId";
    ...
    public function SearchWiki(array $parameter) {
        ...
        $offset = $size * ($page - 1);
        $total = NULL;
        $hasvide = $parameter['need_video'] == "false" ? false : true ;
        $mongo = sfContext::getInstance()->getMondongo();
        $wikiRep = $mongo->getRepository('wiki');
        if($hasvide){
            $result = $wikiRep->xun_search("source:qiyi $keyWord", $total, (int)$offset, (int)$size,null,1);
        }else{
        ...
        if(!$result){
            ...
            foreach($result as $res) {
                ...
                $arr = $this->getWikiVideoSource($res, $i, $arr,1,1); //1,1为不显示getWikiVideoSource后面的信息
                $i++;
            }
        }
        return $arr;
	...
	public function GetWikisByWiki($args)
	{
	    ...
	    if ($wiki->getModel() == 'actor') {
	        ...
	        if($movies) {
	            $nodeArray = $this->getErrArray("false",count($movies),$moviesAll);
	            $i = 1;
	        ...
	        if (!empty($tags)) {
	            ...
	            //echo $searchText['condition'];exit;
	            $movies = $this->getSearch($searchText['condition'], $page, $size);
	        }
	        if (isset($movies['result']) && !empty($movies['result'])) {
	            $nodeArray = $this->getErrArray("false",'',$movies['total'],count($movies['result']));
	            $i = 0;
...
//                break; 
     ...
     * @editor lifucang
     */
     private function getWikiVideoSource($wiki,$i,$nodeArray,$type='',$biaozhi=0){
        ...
        $nodeArray['media'][$i]['description'] = $wiki->getContent();
        $cover = $wiki->getCover();
        if ($cover) {
            $nodeArray['media'][$i]['posters']['num'] = 3;
            $nodeArray['media'][$i]['posters']['poster'][0] = array(
	    ...
	    $screen_num = $wiki->getScreenshotsCount();        
        $nodeArray['media'][$i]['screens']= array(
                        'num'    => $screen_num,
                );
        $screens = $wiki->getScreenshotUrls();   
        foreach($screens as $k => $screen)
        {
            $nodeArray['media'][$i]['screens']['screen'][$k]= array(
                            'url'    =>  $screens[$k],
                    ); 
        }
        if($biaozhi==1){
            ...
            if ($model == 'film') {
                ...
                if ($videos != NULL) {
                    foreach ($videos as $video) {
                        ...
                        //$nodeArray=$this->addEpisodesFilm($i,$video,$nodeArray);
                        if ($video->getReferer() == 'qiyi') {
                            $nodeArray['media'][$i]['episodes'] = array(
                                "source" => "奇艺",
                                "num" => 1
                            );
                            $video_id = (string)$video->getId();
                            $nodeArray['media'][$i]['episodes']['episode'][0] = array(
                                "id" => $video_id,//由于数据由此处获取 无需判断传来的eide是否匹配此video_id
                                "index" => 1,
                                "size" => 0,
                                "length" => 0,
                        ...
                        if ($playList->getReferer() == 'qiyi') {
                            ...
                            if($whether_mark)
                            {
                            	 foreach ($videos as $video) {
    	                                $tvconfig = $video->getConfig();
    	                                if((string)$video->getId()==$type['eid'])
    		                            $nodeArray['media'][$i]['episodes']['episode'][$j] = array(
    		                            	//"markid" => (string)$type['markid'],
    		                            	//"marktime" => $type['marktime'],
...
//	 modify by tianzhongsheng-ex@huan.tv 2013-09-05 wiki更具具体模型返回数据
     ...
     * @author wangnan
     */
     public function getMediaBySort($cid,$filters,$page,$size,$order,$needVideo=true)
     {
     	$nodeArray = array();
		$Condition = $this->getSearchText(null,$cid,$filters,$needVideo);
     ...
     * @author guoqiang.zhang
     */
     public function getSearchText($keyword=null,$cid=0,$filters=null,$needVideo=true){
         $tmpstr = $needVideo ? 'source:qiyi' : '';             //待构建搜索文本 默认搜索影视剧中有qiyi视频源的数据
        ...
        if($channel)
        {
	        ...
	        foreach($programs as $key =>$program)
	        {
				$nodeArray['program'][$key] = array(
                    ...
                    'created_at' => date("Y-m-d H:i:s",$program['created_at']->getTimestamp()),
                    'updated_at' => $program['updated_at']?date("Y-m-d H:i:s",$program['updated_at']->getTimestamp()):null,
	            );
			}        	
        }else{
            $nodeArray = $this->getErrArray(1,'该频道不存在');
     ...
     * 广电接口
     * 根据开始结束时间获取频道更新信息
     * @param array(start_time,end_time)
     * @author lifucang 2013-4-2
     */
    public function GetChannelsUpdate($args)
    {
    	$nodeArray = array();
        $start_time=$args['start_time'];
        $end_time=$args['end_time'];
        $mongo = sfContext::getInstance()->getMondongo();
        $list = $mongo->getRepository('ChannelUpdate')->getChannels($start_time,$end_time);
        if($list){
            $nodeArray = $this->getErrArray(0,'', count($list));
    		foreach($list  as $rs)
    		{
    			$nodeArray['channel_code'][] = $rs->getChannelCode();
    		}            
        }else{
            $nodeArray = $this->getErrArray(0,'');
        }
	...
	/**
     * 广电接口
     * 获取指定时间创建的import数据--默认当天
     * @author lifucang 2013-05-27
     */
	public function GetContentImport($args)
	{
		$nodeArray = array();
        $starttime = $args['start_time']?new MongoDate(strtotime($args['start_time'])):new MongoDate(mktime(0, 0, 0, date('m'), date('d'), date('Y')));
        $endtime=$args['end_time']?new MongoDate(strtotime($args['end_time'])):new MongoDate(mktime(23, 59, 59, date('m'), date('d'), date('Y')));
        $id1=isset($args['id1'])?$args['id1']:null;
        $id2=isset($args['id2'])?$args['id2']:null;
        $match=isset($args['match'])?$args['match']:null;
        $mongo = sfContext::getInstance()->getMondongo();
		$Repository = $mongo->getRepository('ContentImport');
        if($match&&$id1&&$id2){
            $imports = $Repository->find(array('query'=>array('_id'=>array('$gte' => new MongoId($id1),'$lte' => new MongoId($id2)),'state_match'=>1)));
        }elseif($id1&&$id2){
            $imports = $Repository->find(array('query'=>array('_id'=>array('$gte' => new MongoId($id1),'$lte' => new MongoId($id2)),'wiki_id'=>array('$exists'=>true))));
        }elseif($id1){
            $imports = $Repository->find(array('query'=>array('_id'=>new MongoId($id1))));
        }else{
            $imports = $Repository->find(array('query'=>array('updated_at'=>array('$gte' => $starttime,'$lte' => $endtime),'wiki_id'=>array('$exists'=>true))));
        }
        ...
        foreach($imports as $import) 
		{
			$nodeArray['imports'][] =  array(
                            ...
                            'state_match'   => $import->getStateMatch()
            );
		}
		return $nodeArray;
	} 
     ...
     * 获取场景推荐
     * 
     * @author superwen 2013-07-17
     */
    public function GetSceneRecommend($args)
    {
        $nodeArray = array();
        $scene = $args['scene'];
        $limit = intval($args['pagesize']) ? intval($args['pagesize']) : 10;
        $scenes = sfConfig::get('app_rec_scenes');
        ...
        $recommends = $Repository->find(array('query'=>array("scene" => $scene,"is_public" => true),
                                              'sort'=>array('created_at' => -1),
                                              "limit" => $limit));
        foreach($recommends as $recommend) {
            $nodeArray['recommends'][] =  array(
                            'id' => (string)$recommend->getId(),
                            'title'  =>$recommend->getTitle(),  
                            'big_pic'   => file_url($recommend->getPic()),
                            'smallpic'   => file_url($recommend->getSmallpic()),
                            'desc'   => $recommend->getDesc(),
	...
	/**
     * 获取删除的维基ID(默认昨天到今天删除的)
     * @author lifucang 
     * @date 2013-10-24
     */
	public function GetWikisDel($args)
	{
		$nodeArray = array();
        $start_time = $args['start_time']?$args['start_time']:date('Y-m-d',strtotime("-1 days"));
        $end_time = $args['end_time']?$args['end_time']:date('Y-m-d H:i:s');
        
        $startTime = new MongoDate(strtotime($start_time));
        $endTime = new MongoDate(strtotime($end_time));;
    	$mongo = sfContext::getInstance()->getMondongo();
		$WikiDeleteRes = $mongo->getRepository('wikiDelete');
        $wikiDels = $WikiDeleteRes->find(array('query'=>array('created_at'=>array('$gte' => $startTime,'$lte' => $endTime))));
		$nodeArray = $this->getErrArray(0,'',count($wikiDels));
        foreach($wikiDels as $wikiDel) 
		{
		    $nodeArray['wikiIds'][]=$wikiDel->getWikiId();
		}
		return $nodeArray;
	} 
	/**
     ...
     * @date 2013-11-11
     */
    private function GetChannelsByNames($param) 
	{ 		
        $nodeArray = array();
        $names=$param['names'] ? $param['names'] : null;
        if(empty($names)){
    		return $this->getErrArray(1,'请填写names');
        }else{
            $channels =array();
            foreach($names as $key=>$name){
                $channels[$key] = Doctrine::getTable('Channel')->getChannlesByNames($name);
            }
        }
		if($channels) {
			$nodeArray = $this->getErrArray(0,'');
	...
	/**
     * 根据多个频道code获取节目列表
     * @param $param channel_code,start_time,end_time
     * @author lifucang
     * @date 2013-11-11
     */
    private function GetProgramsByChannels($param) 
	{        
    	$codes = $param["codes"];
        $now = date('Y-m-d H:i:s');
    	$starttime = isset($param['start_time'])?$param['start_time']:$now;
    	$endtime = isset($param['end_time'])?$param['end_time']:$now;

        if(empty($codes)){
    		return $this->getErrArray(1,'请填写codes');
        }
    	$mongo = sfContext::getInstance()->getMondongo();
    	$ProgramRepository = $mongo->getRepository('Program');
        $channelTable = Doctrine_Core::getTable("channel");
        $nodeArray = $this->getErrArray(0,'');
        
        foreach($codes as $channelcode){
        	$programs = $ProgramRepository->getProgramsByCode($channelcode,$starttime,$endtime);
            $channel = $channelTable->findOneByCode($channelcode);
            if($channel){
                $channelCode = $channel->getCode();
    			$channelName = $channel->getName();
                $channelLogo = $channel->getLogoUrl();
    	        foreach($programs as $program)
    	        {
    				$nodeArray['programs'][] = array(
    					...
    					'end_time' => date("H:i",$program['end_time']->getTimestamp()),
    					'wiki_id' => $program['wiki_id'],
    					'wiki_cover' => $program->getWikiCoverUrl(),
    					'tags' => $program['tags'],
    	            );                
    			}        	
            }
