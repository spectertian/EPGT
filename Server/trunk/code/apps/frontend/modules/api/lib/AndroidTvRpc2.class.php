...
class AndroidTvRpc2
{
    ...
    public function getNowPrograms($args)
    {
        ...
        if ($programs) {
            ...
            foreach ($programs as $key => $program) {
                ...
                $channel['programs'][$key]['channel_name'] = $channel_info->getName();
                $channel['programs'][$key]['channel_logo'] = $channel_info->getLogoUrl();
    ...
    public function getLiveList(array $args)
    {
        ...
        $memcache_key = "Rpc2_getLiveList_".$province."_".$tag;
        $program_list = $memcache->get($memcache_key);
        if(!$program_list) {
            $mongo = sfContext::getInstance()->getMondongo();
            $programRes = $mongo->getRepository('program');

            $program_lists = $programRes->getCityLiveProgramByTag('', $province, $tag, 30);
            ...
            $memcache->set($memcache_key, $program_list, 600);
        }
        return $program_list;
    }
    ...
    public function getWikiAllInfo($args)
    {
        ...
        $wiki['director'] = $wikiInfo->getDirector(); 
        $wiki['writer'] = $wikiInfo->getWriter(); 
        $wiki['starring'] = $wikiInfo->getStarring(); 
        $wiki['screen'] = $wikiInfo->getScreenshotUrls(); 
     ...
     * @param object $wiki 
     */
    private function getEpisodes($wiki)
    {
        $wiki_episode = array();
        $model = $wiki->getModel();
