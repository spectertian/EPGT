...
class Simple2 
{
    ...
    public function getLctVideoSource($wiki,$i,$nodeArray)
    {
        ...
        */
        ...
        $cover = $wiki->cover;
        if ($cover) {
            $nodeArray['data'][0]['media'][$i]['posters'][$i][DOM::ATTRIBUTES]['num'] = 4;
            ...
            $nodeArray['data'][0]['media'][$i]['posters'][$i]['poster'][3][DOM::ATTRIBUTES] = array(
                    "type" => "hd",
                    "size" => "460*700",
                    "url" => thumb_url($cover, 460, 700),
            );
        }
        /*暂时不取videos信息
        $model = $wiki->model;
...
//      $wiki = $wikiRepository->findOneById(new MongoId($mid));
    ...
    public function GetChannelList($args)
    {
    	...
    	$allowTypes = array('cctv','tv','edu','local','all');
    	$type       =  in_array($type, $allowTypes) ? $type : 'all';
        ...
        $allowSorts = array("0","1","2","3");
        $sort       = in_array($sort, $allowSorts) ? intval($sort) : 0;
        $offset     = ($page -1)*$pagesize;
        $showlive   = intval($args['parameter'][0]['data'][0]['__attributes__']['showlive']);
        
    	switch($type) {
    		case 'cctv':
    			$channels = Doctrine::getTable('Channel')->getYangShiChannels($offset, $pagesize, $sort);
    			break;
    		case 'tv':
    			$channels = Doctrine::getTable('Channel')->getWeiShiChannels($offset, $pagesize, $sort);
    			break;
    		case 'edu':
    			$channels = Doctrine::getTable('Channel')->getEduChannels($offset, $pagesize, $sort);
    			break;
    		case 'local':
    			$channels = Doctrine::getTable('Channel')->getLocalChannels('',$province,$offset,$pagesize, $sort);
    			break;
    		case 'all':
	    		$channels = Doctrine::getTable('Channel')->getUserChannels('', $province, $sort, $offset, $pagesize);
    			break;
    	}
	            ...
	            if($showlive) {
	                ...
	                if($liveprogram) {
	                    $nodeArray['data'][0]['channel'][$key]['program'][0][DOM::ATTRIBUTES] = array(
	                        ...
	                        'wiki_cover' => thumb_url($liveprogram['wiki_cover'], 108, 160),
	...
	public function GetLivePrograme($args)
	{
		...
		if($type != '2')
			$programs = $ProgramRepository->getCityPrograms($city,$province,$tags,$starttime,$endtime,$page,$pagesize);
		else
			$programs = $ProgramRepository->getCityStartPrograms($city,$province,$tags,$starttime,$endtime,$page,$pagesize);
     ...
     * 
     * @editer gaobo 
     * 此接口不再直接入库，而是写入缓存，计划任务（tv:UpdateChannelHot）每五分钟将此接口生成的缓存数据写入数据库
     */
	public function ReportUserLivingAction($args)
	{
	    ...
	    $cacheArray[$userid] = $channel; 
		$memcache->set($memcache_key,$cacheArray);
    ...
    {   
        ...
        */
        ...
        foreach($channels  as $key =>$channel) {
        	if(in_array($channel->getCode(),$chs)) { 
		...
		if(!empty($channels_prior)) {
            return (object) array_merge($channels_prior,$channels_after); //合并数组对象
		} else {
			return $channels;
		}
    }
    ...
    private function GetProgramRec($args)
    {
        ...
        $programRec  = $mongo->find(array('query'=>array('date'=>$nowdate),'sort'=>array('start_time'=>1)));
        if($programRec) {            	
