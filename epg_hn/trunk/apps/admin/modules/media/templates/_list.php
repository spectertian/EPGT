...
$(document).ready(function(){
	...
	$('#chaxun').click(function(){
		$(".content_inner form").attr('action',"<?php echo url_for('media/category_files')?>");
		document.adminForm.submit();
		$(".content_inner form").attr('action','/media/batch/action');
...
<div class="widget">
            ...
            <div class="widget-body file-manager">
              ...
              <?php foreach ($pager->getResults() as $attachment): ?>
                <li class="actived">
                  <div class="thumb">
                      <a href="<?php echo $attachment->getFileUrl() ?>" onclick="return false;" class="img-preview" title="<?php echo $attachment->getSourceName() ?>">
                          <img src="<?php echo thumb_url($attachment->getFileName(),120,120)?>" alt="<?php echo $attachment->getSourceName() ?>">
                      </a>
                  </div>
                  <?php if(!$popup):?>
                  <div class="action"><input type="checkbox" value="<?php echo $attachment->getId() ?>" name="ids[]">
                      ...
                      |&nbsp; <a rel="#" href="<?php echo url_for('@attachments'). '/delete?id='.$attachment->getId() ?>" class="delete"  onClick="return window.confirm('确定删除吗?');">删除
                    </a>
                  </div>
                  <?php endif;?>
			  ...
			  <span class="first-page">
                   ...
                   <a href="<?php echo url_for('media/category_files?page='.$pager->getFirstPage()."&category_id=".$category_id."&popup=".$popup."&source_name=".$source_name);?>">
                        最前页
                    </a>
                </span>
                <span class="prev-page">
                    <a href="<?php echo url_for('media/category_files?page='.$pager->getPreviousPage()."&category_id=".$category_id."&popup=".$popup."&source_name=".$source_name);?>">
                    上一页
                    </a>
                </span>
                <span class="pages">
                  <?php foreach ($pager->getLinks(5) as $key => $value):?>
                    ...
                    <?php else:?>
                        <a href="<?php echo url_for('media/category_files?page='.$value."&category_id=".$category_id."&popup=".$popup."&source_name=".$source_name);?>"><?php echo $value;?></a>
                    <?php endif;?>
                <?php endforeach;?>
                </span>
                <span class="next-page">
                    <a href="<?php echo url_for('media/category_files?page='.$pager->getNextPage()."&category_id=".$category_id."&popup=".$popup."&source_name=".$source_name);?>">
                        下一页
                    </a>
                </span>
                <span class="last-page">
                    <a href="<?php echo url_for('media/category_files?page='.$pager->getLastPage()."&category_id=".$category_id."&popup=".$popup."&source_name=".$source_name);?>">
                        最末页
                    </a>
                </span>
