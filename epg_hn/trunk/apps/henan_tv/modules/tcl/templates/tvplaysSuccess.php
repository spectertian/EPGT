    ...
    */
    ...
    $('.grid').grid({
        ...
        enter: function(event, item) {
            ...
            var wiki_url = '<?php echo url_for('wiki/detail?id='); ?>' + wiki_id;// + '/<?php // echo 'program_id/' ?>' + program_id;
            $("#wiki-sizer").load(wiki_url, function() {
                $("#main-nav-sizer").addClass("display-none");
                $("#wiki-sizer").attr('return_target', '.grid');
                $("#wiki-sizer").removeClass("display-none");
        ...
        change: function(event, ui) {
            ...
            var timeleft = item.attr('timeleft');
            $('#footer_info_title').text(title);
            $('.track').css('width', progress + '%');
            $(".footer .time-left").text(timeleft);
            $('.info').show();
        },
...
<div class="cat-list">
    <div class="grid">
        ...
        <?php foreach ($programs as $program): ?>
            ...
            <?php if(array_key_exists($program->getWikiId(),$WikiPro) && $WikiPro) continue;?>
            <?php $WikiPro[$program->getWikiId()] = $program->getWikiId();?>
            <li
                class="action"
                ...
                timeleft="剩余时间: <?php echo $program->getLiftTime() ?>"
                program_id="<?php echo $program->getId(); ?>"
            >
                <div class="item">
                    <div class="cover">
                        <img src="<?php echo thumb_url($program->getWikiCover(),166,191); ?>" width="" height="" alt="" />
                    </div>
