...
//    }
    ...
    * @date 2012-8-20
    */
    public function findChannelByCode($channelcode) {
        $memcache = tvCache::getInstance();
        $memcache_key = md5($channelcode);
        $channel = $memcache->get($memcache_key);
        if(!$channel){
            $channel_query =   $this->createQuery()
                ->where('code = ?', $channelcode)
                ->andWhere('publish = ?', 1);
            $channel = $channel_query->execute();
            $memcache->set($memcache_key,$channel);
        }
        return $channel;
    }
}
