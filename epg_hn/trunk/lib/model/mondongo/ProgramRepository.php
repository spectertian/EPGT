...
class ProgramRepository extends \BaseProgramRepository
{
    ...
    /**
     * 获取指定频道列表，当前正在直播的某TAG节目，专为电视端前台显示，要求拥有 Wiki (河南专用)
     * @param string $tag
     * @param array $channels
     * @return array
     * @modified zhigang 2011-1-4
     */
    public function getLiveProgramByTagHn($tag, $channels="",$limit = 0) {
        $channel_codes = array();
        foreach ($channels as $channel) {
     ...
     * @author lifucang
     */
    public function getDayPlayedProgramByWikiIdGd($wiki_id,$limit=0) {
        $now = new MongoDate();
        $date_start = new MongoDate(mktime(0, 0, 0, date('m'), date('d'), date('Y')));
        $date_end=new MongoDate(mktime(23, 59, 0, date('m'), date('d'), date('Y')));
        $rets = $this->find(
     ...
     * @author lifucang
     */
    public function getDayUnPlayedProgramByWikiIdGd($wiki_id,$limit=0) {
        $now = new MongoDate();
        $date_start = new MongoDate(mktime(0, 0, 0, date('m'), date('d'), date('Y')));
        $date_end=new MongoDate(mktime(23, 59, 0, date('m'), date('d'), date('Y')));
        $rets = $this->find(
     ...
     * @author lifucang
     */
    public function getProgramByWikiIdTimeHn($wiki_id,$start_time,$end_time,$limit=0) {
        $now = new MongoDate();
        ...
        $spRep = $mondongo->getRepository('spService');
        
		if ($programs){
			foreach ($programs as $program) {
				$channelcode = $program -> getChannelCode();
				$sp = $spRep->findOne(array('query'=>array('channel_code'=>$channelcode)));
				if ($sp){
					$array[]=$program;
				}
			}
    ...
    /**
     * 通过wiki获取当前正在播放的节目河南专用 (电视台加频道过滤  只显示已与serviceid关联的电视台)
     * @author lifucang
     */
    public function getLiveProgramByWikiHN($wiki_id) {
        $now = new MongoDate();
        $mondongo = $this->getMondongo();
        $spRep = $mondongo->getRepository('spService');
