...
EOF;
    ...
    { 
        ...
        foreach($channels as $channel){
            $nodeArray=array(); 
            //$file='log/ADI_'.iconv("UTF-8","GBK",$channel->getName()).'.xml';
            ...
            $nodeArray['Metadata'][0]['AMS'][0][DOM::ATTRIBUTES]=array(
                   'Verb'=>'',
                   'Asset_Class'=>'package',
                   'Asset_ID'=>str_pad((string)$channel->getId(),22,"0",STR_PAD_LEFT),
                   'Asset_Name'=>$channel->getName(),
                   'Provider_ID'=>'ngcp',
                   'Creation_Date'=>$date,
                   'Description'=>$channel->getMemo(),
                   'Version_Major'=>'1',
                   'Version_Minor'=>'2',
                   'Product'=>'MOD',
            );
            ...
            $nodeArray['Asset'][0]['Metadata'][0]['App_Data'][2][DOM::ATTRIBUTES]=array(
                   'Value'=>str_pad((string)$channel->getId(),3,"0",STR_PAD_LEFT),
                   'Name'=>'Channel_Number',
                   'App'=>'MOD',
            );
            ...
            foreach($programs as $program){
                $file='log/ADI/ADI_'.iconv("UTF-8","GBK",$program->getName()).'.xml';
                @unlink($file);
                ...
                $nodeArray['Asset'][0]['Asset'][$k]['Metadata'][0]['App_Data'][0][DOM::ATTRIBUTES]=array(
	                   'App'=>'MOD',
                       'Name'=>'Channel_ID',
                       'Value'=>str_pad((string)$channel->getId(),22,"0",STR_PAD_LEFT),
                );
                ...
                if($wiki){
                       ...
                       $nodeArray['Asset'][0]['Asset'][$k]['Metadata'][0]['App_Data'][7][DOM::ATTRIBUTES]=array(
                           'App'=>'MOD',
                           'Name'=>'Genre',
                           'Value'=>!$wiki->getTags() ? '' : implode(',', $wiki->getTags()),
                       );   
